...
import { OpenVoiceProvider } from './audioProviders/openVoiceProvider';
import { ElevenLabsProvider } from './audioProviders/elevenLabsProvider';
...
export class AudioService {
  ...
  private initializeProviders(): void {
    ...
    this.providers.set(AudioProviderType.OPENVOICE, new OpenVoiceProvider());
    this.providers.set(AudioProviderType.ELEVENLABS, new ElevenLabsProvider());

    // Add more providers here as they become available
    // this.providers.set(AudioProviderType.COQUI_TTS, new CoquiProvider());
    // this.providers.set(AudioProviderType.HUGGINGFACE, new HuggingFaceProvider());
  }

  async switchProvider(providerType: AudioProviderType, config?: AudioProviderConfig): Promise<void> {
  ...
  async cloneCharacterVoice(
    ...
    try {
      ...
      const audioUrl = await this.uploadAudioToSupabase(referenceAudio, `characters/${characterId}/reference`);
  ...
  private async uploadAudioToSupabase(audioFile: File | Blob, path: string): Promise<string> {
    const file = audioFile instanceof File ? audioFile : new File([audioFile], 'audio.mp3', {
      type: 'audio/mpeg'
    });
