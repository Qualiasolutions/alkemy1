...
class NetworkDetectionService {
    ...
    private initialize() {
        ...
        console.log('[Network Detection] Initialized. Current status:', this.isOnline ? 'Online' : 'Offline');
    }
    ...
    throwIfOffline(operation: string = 'this operation') {
        if (!this.isOnline) {
            throw new Error(`Cannot perform ${operation} - you appear to be offline. Please check your internet connection and try again.`);
        }
    }
