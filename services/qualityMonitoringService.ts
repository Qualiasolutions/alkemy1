...
function checkQualityThresholds(
    ...
    if (qualityReport.colorConsistency < 60) {
        const alert: QualityAlert = {
            ...
            suggestion: 'Use a consistent color grading LUT or establish a stronger color palette in your prompts.',
            timestamp: new Date()
        };
    ...
    if (qualityReport.lightingCoherence < 60) {
        const alert: QualityAlert = {
            ...
            suggestion: 'Standardize lighting descriptions (e.g., "golden hour", "soft daylight") across related scenes.',
            timestamp: new Date()
        };
    ...
    qualityReport.sceneReports.forEach(scene => {
        if (scene.overallScore < 50) {
            const alert: QualityAlert = {
                ...
                suggestion: 'Consider regenerating this scene with refined prompts or better reference images.',
                actionRequired: true,
                timestamp: new Date(),
...
function checkCostThresholds(
    ...
    if (performanceMetrics.apiCosts.totalProjectCost > monitoringState.costThreshold) {
        const alert: QualityAlert = {
            id: `cost-${Date.now()}`,
            severity: performanceMetrics.apiCosts.totalProjectCost > monitoringState.costThreshold * 2 ? 'critical' : 'warning',
            category: 'cost',
            ...
            suggestion: 'Consider using more cost-effective models for drafts or reducing the number of variations.',
            actionRequired: performanceMetrics.apiCosts.totalProjectCost > monitoringState.costThreshold * 2,
            timestamp: new Date()
        };
    ...
    if (performanceMetrics.efficiencyMetrics.successRate < 70) {
        const alert: QualityAlert = {
            ...
            suggestion: 'Review failed generations to identify common issues. Consider simplifying prompts or checking API quotas.',
            timestamp: new Date()
        };
...
function generateProactiveSuggestions(
    ...
    if (qualityReport.lightingCoherence < 80) {
        suggestions.push({
            id: `lighting-${Date.now()}`,
            category: 'lighting',
            suggestion: 'Try using consistent lighting keywords like "soft diffused light" or "dramatic chiaroscuro" across scenes.',
            impact: 'high',
            autoApplicable: true,
            parameters: {
    ...
    if (qualityReport.colorConsistency < 80) {
        suggestions.push({
            id: `color-${Date.now()}`,
            category: 'color',
            suggestion: 'Establish a color palette. Consider: warm earth tones, cool blues, or desaturated noir palette.',
            impact: 'high',
            autoApplicable: true,
            parameters: {
    ...
    const needsCompositionHelp = qualityReport.sceneReports.some(
        scene => scene.overallScore < 60
    );
    ...
    const charactersNeedingIdentity = scriptAnalysis.characters.filter(
        char => (!char.identity || char.identity.status !== 'ready') &&
                char.scenes && char.scenes.length > 2
    );
...
function addAlert(alert: QualityAlert, onAlert?: (alert: QualityAlert) => void): void {
    ...
    if (!existingAlert ||
        (Date.now() - existingAlert.timestamp.getTime()) > 300000) { // Re-alert after 5 minutes
...
export function getActiveAlerts(): QualityAlert[] {
    ...
    return monitoringState.alertQueue.filter(
...
export function dismissAlert(alertId: string): void {
    monitoringState.alertQueue = monitoringState.alertQueue.filter(
    ...
    try {
        ...
        if (suggestion.parameters) {
            ...
            localStorage.setItem(
                'quality-suggestion-params',
                JSON.stringify(suggestion.parameters)
            );
        }

        // Remove applied suggestion
        monitoringState.suggestions = monitoringState.suggestions.filter(
            s => s.id !== suggestionId
        );
