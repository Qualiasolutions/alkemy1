...
export const generateVideoFromTextWan = async (
    ...
    try {
        if (!prefersLiveWan()) {
            ...
            onProgress?.(100);
            const [fallbackBlob] = await getFallbackVideoBlobs(1, `wan-text-fallback-${prompt.substring(0, 30)}`);
            return URL.createObjectURL(fallbackBlob);
        }
...
export const generateVideoFromImageWan = async (
    ...
    try {
        if (!prefersLiveWan()) {
            console.warn('[Wan API] Fallback mode active – returning stock video.');
            onProgress?.(100);
            const [fallbackBlob] = await getFallbackVideoBlobs(1, `wan-image-fallback-${prompt.substring(0, 30)}`);
            return URL.createObjectURL(fallbackBlob);
        }
