...
import { UsersIcon, MapPinIcon, ArrowLeftIcon, AlkemyLoadingIcon, XIcon, PlusIcon, ImagePlusIcon, Trash2Icon, PaperclipIcon, ExpandIcon, CheckCircleIcon, AlertCircleIcon, UploadIcon, SparklesIcon } from '../components/icons/Icons';
...
const GenerationView: React.FC<{
    ...
    const [model, setModel] = useState<'Imagen' | 'Gemini Nano Banana' | 'Flux' | 'Flux Kontext Max Multi'>('Flux');
    ...
    const [imageSlots, setImageSlots] = useState<ImageSlot[]>([
        { id: 'main', label: 'Main', priority: 1, url: item.data.imageUrl, generation: null, isLoading: false, progress: 0 },
        { id: 'secondary', label: 'Secondary', priority: 2, url: null, generation: null, isLoading: false, progress: 0 },
        { id: 'tertiary', label: 'Tertiary', priority: 3, url: null, generation: null, isLoading: false, progress: 0 },
        { id: 'fourth', label: 'Fourth', priority: 4, url: null, generation: null, isLoading: false, progress: 0 },
    ]);
    ...
    const handleGenerate = async () => {
        ...
        setImageSlots(prev => {
            const updated = [...prev];
            loadingGenerations.forEach((gen, idx) => {
            ...
            <header className="flex-shrink-0 px-8 py-6 border-b border-gray-800/80 backdrop-blur-xl bg-black/40">
                <div className="flex items-center justify-between gap-6 mb-6">
                    ...
                    <div className="flex-1 text-center">
                        ...
                        <p className="text-sm text-gray-400">{item.type === 'character' ? 'Character' : 'Location'} Generation Studio</p>
                    </div>
                ...
                <main className="flex-1 overflow-hidden flex gap-6 p-8">
                    ...
                    <aside className="w-96 flex flex-col gap-4">
                        <div className="bg-gradient-to-br from-gray-900/90 to-gray-800/90 rounded-2xl border border-gray-700/50 p-4 backdrop-blur-sm">
                            <h3 className="text-sm font-bold text-gray-300 uppercase tracking-wider mb-1">Image Variants</h3>
                            ...
                            <div className="space-y-3">
                                {imageSlots.map((slot, idx) => (
                                    ...
                                    >
                                            ...
                                            {slot.isLoading ? (
                                                <div className="absolute inset-0 flex flex-col items-center justify-center">
                                                    ...
                                                    <div className="w-3/4 bg-gray-700 rounded-full h-1">
                                                        <div className="bg-teal-500 h-1 rounded-full transition-all" style={{ width: `${slot.progress}%` }}></div>
                                                    </div>
                                        ...
                                        {/* Slot Label */}
                                        <div className={`absolute top-2 left-2 px-2.5 py-1 rounded-lg text-xs font-bold backdrop-blur-md ${
                                            slot.id === 'main'
                    ...
                    <div className="flex-1 flex items-center justify-center bg-gradient-to-br from-black/60 to-gray-900/40 rounded-3xl border-2 border-gray-800/50 p-12 backdrop-blur-sm">
                        {selectedSlotData?.url ? (
                            ...
                            <div className="text-center">
                                ...
                                <p className="text-gray-300 text-lg font-semibold">Generating {selectedSlotData.label}...</p>
                                <p className="text-gray-500 text-sm mt-2">{Math.round(selectedSlotData.progress)}% complete</p>
                            </div>
                        ) : (
                            <div className="text-center">
                                <ImagePlusIcon className="w-20 h-20 text-gray-600 mx-auto mb-4" />
                                <p className="text-gray-400 text-lg mb-2">No image in {selectedSlotData?.label} slot</p>
                                ...
                                <div className="p-6 space-y-5">
                                    ...
                                    <AnimatePresence>
                                        {attachedImage && (
                                            ...
                                            >
                                                <img src={attachedImage} alt="Reference" className="w-24 h-24 object-cover rounded-xl" />
                                                <motion.button
                                    ...
                                    <div className="flex items-center justify-between gap-4 flex-wrap">
                                        <div className="flex items-center gap-3 flex-wrap">
                                            ...
                                            {isCharacter && character?.identity && getCharacterIdentityStatus(character.identity) === 'ready' && (
                                                <motion.div
                                                ...
                                                >
                                                    ...
                                                    <span className="text-sm font-semibold text-emerald-300">Identity Active</span>
                                                </motion.div>
                                            )}
                                        </div>
