    ...
    return (
      <div className="flex items-center justify-center h-full">
        <div className="text-center">
          <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-#dfec2d/10 flex items-center justify-center">
            <svg className="w-8 h-8 text-#dfec2d" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    {frame.status && (
                      <div className="absolute top-2 right-2">
                        <span className={`px-2 py-1 text-xs rounded-full ${
                          frame.status === 'UpscaledVideoReady' ? 'bg-#dfec2d/20 text-#dfec2d' :
                          frame.status === 'VideoReady' ? 'bg-[var(--color-accent-secondary)]/20 text-[var(--color-accent-secondary)]' :
                          frame.status === 'GeneratingStill' ? 'bg-#dfec2d/20 text-#dfec2d' :
      ...
      {isLoading && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-gray-900 rounded-lg p-6 flex items-center gap-3">
            <div className="w-6 h-6 border-2 border-#dfec2d border-t-transparent rounded-full animate-spin"></div>
