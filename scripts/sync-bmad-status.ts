...
if (!supabaseUrl || !supabaseKey) {
  console.error(
    '❌ Missing Supabase configuration. Please set VITE_SUPABASE_URL and VITE_SUPABASE_SERVICE_ROLE_KEY'
  )
  process.exit(1)
}
...
function parseAcceptanceCriteria(content: string): AcceptanceCriterion[] {
  ...
  let match: RegExpExecArray | null = acRegex.exec(content)
  while (match !== null) {
    criteria.push({
      number: `AC${match[2]}`,
...
function parseIntegrationVerifications(content: string): IntegrationVerification[] {
  ...
  let match: RegExpExecArray | null = ivRegex.exec(content)
  while (match !== null) {
    verifications.push({
      number: `IV${match[2]}`,
      description: match[3].trim(),
...
function parseMigrationCheckpoints(content: string): MigrationCheckpoint[] {
  ...
  let match: RegExpExecArray | null = mcRegex.exec(content)
  while (match !== null) {
    checkpoints.push({
      number: `MC${match[2]}`,
      description: match[3].trim(),
...
async function getOrCreateEpicId(epicNumber: string): Promise<string | null> {
  ...
  const { data } = await supabase.from('epics').select('id').eq('epic_number', epicNumber).single()
...
async function syncStoryFile(filePath: string): Promise<void> {
  ...
  try {
    ...
    for (const ac of acceptanceCriteria) {
      await supabase.from('acceptance_criteria').upsert(
        {
          story_id: storyId,
    ...
    for (const iv of integrationVerifications) {
      await supabase.from('integration_verifications').upsert(
        {
          story_id: storyId,
    ...
    for (const mc of migrationCheckpoints) {
      await supabase.from('migration_checkpoints').upsert(
        {
          story_id: storyId,
    ...
    console.log(
      `  ✅ Synced successfully (${acceptanceCriteria.length} AC, ${integrationVerifications.length} IV, ${migrationCheckpoints.length} MC)`
    )
  } catch (error) {
...
${
  stories
    ...
    .map((story) => `- **${story.story_number}**: ${story.title} - ⚠️ BLOCKED`)
    .join('\n') || '*(No blocked stories)*'
}
...
${
  stories
    ...
    .slice(0, 5)
    .map((story) => `- **${story.story_number}**: ${story.title} ✅`)
...
// Main execution
async function main() {
  console.log('🚀 Starting BMAD sync script...')
  try {
    // Sync all stories
