...
const ExportPanel: React.FC<ExportPanelProps> = ({
    ...
    const exportOptions = dataType === 'character' ? characterExportOptions :
                         dataType === 'world' ? worldExportOptions :
    ...
    return (
        <div className={`p-6 rounded-xl ${className}`} style={{ background: colors.surface_card, borderWidth: '1px', borderStyle: 'solid', borderColor: colors.border_color }}>
            ...
            <div className="space-y-3 mb-6">
                {exportOptions.map((option) => (
                    <motion.button
                        ...
                        style={{
                            background: selectedFormat === option.format ? 'rgba(16, 163, 127, 0.1)' : colors.background_secondary,
                            borderWidth: '2px',
                            borderStyle: 'solid',
                            borderColor: selectedFormat === option.format ? colors.accent_primary : colors.border_color
                        }}
                        whileHover={{ scale: 1.02 }}
                        whileTap={{ scale: 0.98 }}
                    >
                        <div className="flex items-start gap-3">
                            <div className="flex-shrink-0 mt-1" style={{ color: selectedFormat === option.format ? colors.accent_primary : colors.text_secondary }}>
                                {option.icon}
                            </div>
