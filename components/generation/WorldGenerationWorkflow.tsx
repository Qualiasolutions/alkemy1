...
import { worldLabsService, GeneratedWorld, WorldGenerationRequest } from '@/services/worldLabsService';
        ...
        <div className="space-y-6">
            {!generatedWorld ? (
                <motion.div
                    ...
                    className="space-y-6"
                >
                    <div className="p-6 rounded-xl" style={{ background: colors.surface_card, borderWidth: '1px', borderStyle: 'solid', borderColor: colors.border_color }}>
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <textarea
                                        ...
                                        style={{ background: colors.background_secondary, borderWidth: '1px', borderStyle: 'solid', borderColor: colors.border_color, color: colors.text_primary }}
                                    />
                                </div>

                                {/* Demo Prompts */}
                                <div>
                                    <p className="text-sm font-semibold mb-2" style={{ color: colors.text_secondary }}>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                        {DEMO_WORLD_PROMPTS.slice(0, 4).map((demo) => (
                                            <button
                                                ...
                                                style={{ background: colors.background_secondary, borderWidth: '1px', borderStyle: 'solid', borderColor: colors.border_color }}
                                            >
                                                <p className="font-semibold mb-1" style={{ color: colors.text_primary }}>{demo.name}</p>
                                                <p className="text-xs line-clamp-2" style={{ color: colors.text_tertiary }}>{demo.prompt}</p>
                                            </button>
                                        ))}
                                    </div>
                                </div>
                            ...
                            <div>
                                ...
                                >
                                    {imagePreview ? (
                                        ...
                                        <>
                                            <svg className="mx-auto mb-4" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke={colors.text_secondary} strokeWidth="2">
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                {(['draft', 'standard', 'ultra'] as const).map((q) => (
                                    <button
                                        ...
                                        style={{
                                            background: quality === q ? 'rgba(16, 163, 127, 0.1)' : colors.background_secondary,
                                            borderWidth: '2px',
                                            borderStyle: 'solid',
                                            borderColor: quality === q ? colors.accent_primary : colors.border_color
                                        }}
                                    >
                                        <p className="font-bold capitalize" style={{ color: colors.text_primary }}>{q}</p>
                        ...
                        {/* Feature Toggles */}
                        <div className="mt-6 p-4 rounded-lg space-y-3" style={{ background: colors.background_secondary }}>
                            <p className="font-semibold mb-3" style={{ color: colors.text_primary }}>
                                World Features
                            </p>
                            {[
                                { key: 'physics', label: 'Physics Simulation', value: enablePhysics, setter: setEnablePhysics },
                                { key: 'lighting', label: 'Dynamic Lighting', value: enableLighting, setter: setEnableLighting },
                                { key: 'interactivity', label: 'Interactive Elements', value: enableInteractivity, setter: setEnableInteractivity }
                            ].map((feature) => (
                    ...
                    {/* Generate Button */}
                    {isGenerating ? (
                        <div className="p-12 rounded-xl text-center" style={{ background: colors.surface_card }}>
                            <motion.div
                                animate={{ rotate: 360 }}
                            ...
                            <p className="mb-6" style={{ color: colors.text_secondary }}>{generationStatus}</p>
                            <div className="max-w-md mx-auto">
                                <div className="h-2 rounded-full overflow-hidden" style={{ background: colors.border_color }}>
                                    <motion.div
                                ...
                                <p className="text-sm mt-2" style={{ color: colors.text_tertiary }}>{generationProgress}%</p>
                            </div>
                        </div>
                    ) : (
                        <Button
                ...
                >
                    ...
                    <div className="p-6 rounded-xl" style={{ background: colors.surface_card, borderWidth: '1px', borderStyle: 'solid', borderColor: colors.border_color }}>
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <div className="flex gap-2">
                                <div className="px-3 py-1 rounded text-sm font-semibold" style={{ background: 'rgba(16, 163, 127, 0.2)', color: colors.accent_primary }}>
                                    {quality.toUpperCase()} Quality
                                </div>
                            </div>
                        </div>

                        <div className="relative w-full aspect-video rounded-lg overflow-hidden" style={{ background: colors.background_primary }}>
                            <canvas ref={canvasRef} className="w-full h-full" />
                            <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between">
                                <div className="px-4 py-2 rounded-lg backdrop-blur-md" style={{ background: 'rgba(0, 0, 0, 0.6)' }}>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
                            <div className="p-3 rounded-lg" style={{ background: colors.background_secondary }}>
                                <p className="text-xs font-semibold uppercase mb-1" style={{ color: colors.text_tertiary }}>Gaussian Splats</p>
                            ...
                            <div className="p-3 rounded-lg" style={{ background: colors.background_secondary }}>
                                <p className="text-xs font-semibold uppercase mb-1" style={{ color: colors.text_tertiary }}>Physics Objects</p>
                                <p className="text-lg font-bold" style={{ color: colors.text_primary }}>
                                    {generatedWorld.metadata.physicsWorld.dynamicObjects.length}
                                </p>
                            </div>
                            <div className="p-3 rounded-lg" style={{ background: colors.background_secondary }}>
                                <p className="text-xs font-semibold uppercase mb-1" style={{ color: colors.text_tertiary }}>Interactions</p>
                                <p className="text-lg font-bold" style={{ color: colors.text_primary }}>
                                    {generatedWorld.metadata.interactionPoints.length}
                                </p>
                            </div>
                            <div className="p-3 rounded-lg" style={{ background: colors.background_secondary }}>
                                <p className="text-xs font-semibold uppercase mb-1" style={{ color: colors.text_tertiary }}>Render Mode</p>
                    ...
                    <div className="grid md:grid-cols-3 gap-6">
                        <div className="md:col-span-2">
                            <Button onClick={() => setGeneratedWorld(null)} variant="secondary" className="w-full">
