...
import { characterIdentityService, TrainedCharacterIdentity, GeneratedCharacterImage } from '@/services/characterIdentityService';
import { DEMO_CHARACTERS } from '@/data/generationDemoData';
import ResultsGallery, { GalleryItem } from './ResultsGallery';
        ...
        <div className="space-y-6">
            ...
            {step === 'upload' && (
                ...
                >
                    <div className="p-6 rounded-xl" style={{ background: colors.surface_card, borderWidth: '1px', borderStyle: 'solid', borderColor: colors.border_color }}>
                        ...
                        {/* Demo Mode Toggle */}
                        <div className="mb-6 p-4 rounded-lg" style={{ background: 'rgba(16, 163, 127, 0.1)', borderWidth: '1px', borderStyle: 'solid', borderColor: colors.accent_primary }}>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    {DEMO_CHARACTERS.map((char, index) => (
                                        <motion.button
                                            ...
                                            whileTap={{ scale: 0.98 }}
                                        >
                                            <img src={char.referenceUrls[0]} alt={char.name} className="w-full h-32 object-cover rounded mb-3" />
                                            <h3 className="font-bold mb-1" style={{ color: colors.text_primary }}>{char.name}</h3>
                                            <p className="text-xs mb-2" style={{ color: colors.text_tertiary }}>{char.description}</p>
                                            <div className="flex items-center gap-2">
                                                <div className="text-xs px-2 py-1 rounded" style={{ background: 'rgba(16, 163, 127, 0.2)', color: colors.accent_primary }}>
                                ...
                                {previewUrls.length > 0 && (
                                    <div className="grid grid-cols-3 md:grid-cols-5 gap-4 mt-4">
                                        {previewUrls.map((url, index) => (
                                            ...
                                            >
                                                <img src={url} alt={`Preview ${index + 1}`} className="w-full aspect-square object-cover rounded-lg" />
                                                <button
                                                    onClick={(e) => {
                                                        e.stopPropagation();
                    ...
                    >
                        {useDemoMode ? 'Continue with Demo Character' : `Train Character Identity (${selectedFiles.length}/10 images)`}
                    </Button>
                </motion.div>
            )}
            ...
            {step === 'training' && (
                ...
                >
                    ...
                    <p className="mb-6" style={{ color: colors.text_secondary }}>{trainingStatus}</p>
                    <div className="max-w-md mx-auto">
                        ...
                        <p className="text-sm mt-2" style={{ color: colors.text_tertiary }}>{trainingProgress}%</p>
                    </div>
                </motion.div>
            )}
                    ...
                    <div className="p-6 rounded-xl" style={{ background: colors.surface_card, borderWidth: '1px', borderStyle: 'solid', borderColor: colors.border_color }}>
                        <div className="flex items-start justify-between mb-6">
                            <div>
                                ...
                                <div className="flex items-center gap-4 mt-4">
                                    <div className="px-3 py-1 rounded" style={{ background: 'rgba(16, 163, 127, 0.2)', color: colors.accent_primary }}>
                                        {trainedIdentity.consistencyScore.toFixed(1)}% Consistency Score
                                    </div>
                                    <div className="px-3 py-1 rounded" style={{ background: colors.background_secondary, color: colors.text_secondary }}>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                            {Object.entries(trainedIdentity.features).map(([key, value]) => (
                                <div key={key} className="p-3 rounded-lg" style={{ background: colors.background_secondary }}>
                                    <p className="text-xs font-semibold uppercase mb-1" style={{ color: colors.text_tertiary }}>
                        ...
                        {/* Reference Images */}
                        <h3 className="font-bold mb-3" style={{ color: colors.text_primary }}>Reference Images</h3>
                        <div className="grid grid-cols-3 md:grid-cols-5 gap-3">
                            {trainedIdentity.referenceUrls.map((url, index) => (
                                <img key={index} src={url} alt={`Reference ${index + 1}`} className="w-full aspect-square object-cover rounded-lg" />
                            ))}
                        </div>
                    </div>
