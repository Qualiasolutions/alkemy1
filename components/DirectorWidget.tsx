...
const DirectorWidget: React.FC<DirectorWidgetProps> = ({ scriptAnalysis }) => {
  ...
  const widgetLabel = useMemo(() => (canChat ? 'Director' : 'Analyze script to chat'), [canChat]);
        ...
        <div className="relative w-[360px] overflow-hidden rounded-3xl border border-white/10 bg-[#111318]/95 shadow-[0_30px_70px_rgba(3,7,18,0.65)] backdrop-blur-xl">
          <header className="flex items-center justify-between gap-3 border-b border-white/10 px-5 py-4">
            <div className="flex items-center gap-3">
              <span
                className="flex h-9 w-9 items-center justify-center rounded-full bg-[rgba(16,163,127,0.2)]"
            ...
            <div className="flex-1 space-y-4 overflow-y-auto px-5 py-4">
              {messages.map((message, index) => (
                ...
                >
                  {message.author === 'director' && (
                    <span
                      className="mt-0.5 inline-flex h-8 w-8 items-center justify-center rounded-full bg-[rgba(16,163,127,0.15)]"
                      style={{ color: ACCENT_HEX }}
                    >
                      <BrainIcon className="h-4 w-4" />
                    </span>
              ...
              {isLoading && (
                <div className="flex items-center gap-3 text-sm text-white/70">
                  <span
                    className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-[rgba(16,163,127,0.15)]"
                    style={{ color: ACCENT_HEX }}
                  >
