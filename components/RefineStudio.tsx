    ...
    <div className="fixed inset-0 z-50 bg-black bg-opacity-75 flex items-center justify-center">
      <div className="w-full h-full max-w-7xl max-h-screen bg-[var(--color-background-primary)] rounded-xl overflow-hidden">
        ...
        <div className="h-[calc(100vh-80px)] flex flex-col overflow-hidden">
          <div className="m-4 border-2 border-#dfec2d rounded-2xl bg-[var(--color-surface-card)] overflow-hidden flex flex-col flex-1">
              ...
              <div className="w-72 bg-[var(--color-background-secondary)] border-r border-[var(--color-border-color)] flex flex-col p-6 overflow-y-auto">
                ...
                <div className="mb-6 flex-shrink-0">
                  <textarea
                    ...
                    className="w-full h-24 bg-[var(--color-surface-card)] text-[var(--color-text-primary)] placeholder-[var(--color-text-tertiary)] rounded-lg p-3 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-#dfec2d border border-[var(--color-border-color)]"
                  />
                  <Button
                        ...
                        <button
                          ...
                          className={`w-full text-left p-2 rounded-lg text-xs transition ${
                            currentImageId === version.id
                              ? 'bg-#dfec2d/20 text-#dfec2d border border-#dfec2d/40'
                        ...
                        >
                          {version.isLoading ? (
                            <div className="flex items-center">
                              <div className="w-4 h-4 border-2 border-#dfec2d border-t-transparent rounded-full animate-spin mr-2" />
                              Processing...
                            </div>
                          ) : (
                            <>
                    ...
                    <div className="relative w-full h-full bg-[var(--color-surface-card)] rounded-lg overflow-hidden flex items-center justify-center">
                      {currentImage.isLoading ? (
                        <div className="flex flex-col items-center justify-center">
                          <div className="w-12 h-12 border-4 border-#dfec2d border-t-transparent rounded-full animate-spin mb-4" />
                  ...
                  >
                    {version.isLoading ? (
                      <div className="w-full h-full bg-[var(--color-surface-card)] flex items-center justify-center">
                        <div className="w-4 h-4 border-2 border-#dfec2d border-t-transparent rounded-full animate-spin" />
                      </div>
                    ) : (
                      <img
                        src={version.image}
