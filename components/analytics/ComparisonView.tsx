...
export default function ComparisonView({
    ...
    const costChange = currentMetrics && previousMetrics
        ? currentMetrics.apiCosts.totalProjectCost - previousMetrics.apiCosts.totalProjectCost
        : null;

    const successRateChange = currentMetrics && previousMetrics
                ...
                {qualityChange !== null && (
                    ...
                    >
                        ...
                        <div style={{ fontSize: '36px', fontWeight: 'bold', marginBottom: '8px' }}>
                            ...
                            <span style={{ color: qualityChange > 0 ? '#10b981' : qualityChange < 0 ? '#ef4444' : colors.text_primary }}>
                                {currentQuality?.overallScore || 0}
                            </span>
                        </div>
                        <p style={{
                            fontSize: '18px',
                            color: qualityChange > 0 ? '#10b981' : qualityChange < 0 ? '#ef4444' : colors.text_secondary
                        }}>
                            {qualityChange > 0 ? '↑' : qualityChange < 0 ? '↓' : '='} {Math.abs(qualityChange).toFixed(1)} points
                        </p>
                    </motion.div>
                )}
                ...
                {costChange !== null && (
                    ...
                    >
                        ...
                        <div style={{ fontSize: '36px', fontWeight: 'bold', marginBottom: '8px' }}>
                            ...
                            <span style={{ color: colors.text_secondary, margin: '0 12px' }}>→</span>
                            <span style={{ color: costChange > 0 ? '#ef4444' : costChange < 0 ? '#10b981' : colors.text_primary }}>
                                ${currentMetrics?.apiCosts.totalProjectCost.toFixed(2) || 0}
                            </span>
                        </div>
                        <p style={{
                            fontSize: '18px',
                            color: costChange > 0 ? '#ef4444' : costChange < 0 ? '#10b981' : colors.text_secondary
                        }}>
                ...
                {successRateChange !== null && (
                    ...
                    >
                        ...
                        <div style={{ fontSize: '36px', fontWeight: 'bold', marginBottom: '8px' }}>
                            ...
                            <span style={{ color: colors.text_secondary, margin: '0 12px' }}>→</span>
                            <span style={{ color: successRateChange > 0 ? '#10b981' : successRateChange < 0 ? '#ef4444' : colors.text_primary }}>
                                {currentMetrics?.efficiencyMetrics.successRate.toFixed(1) || 0}%
                            </span>
                        </div>
                        <p style={{
                            fontSize: '18px',
                            color: successRateChange > 0 ? '#10b981' : successRateChange < 0 ? '#ef4444' : colors.text_secondary
                        }}>
                            {successRateChange > 0 ? '↑' : successRateChange < 0 ? '↓' : '='} {Math.abs(successRateChange).toFixed(1)}%
                        </p>
                    </motion.div>
                )}
            </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                        {qualityChange && qualityChange > 0 && (
                            <div style={{
                                ...
                                <p style={{ color: colors.text_secondary, fontSize: '14px' }}>
                                    Your overall quality score increased by {qualityChange.toFixed(1)} points.
                                    This indicates better visual consistency and adherence to creative vision.
                                </p>
                            </div>
                        )}
                        ...
                        {costChange && Math.abs(costChange) > 0.01 && (
                            <div style={{
                                ...
                                <p style={{ color: colors.text_secondary, fontSize: '14px' }}>
                                    Project costs {costChange < 0 ? 'decreased' : 'increased'} by ${Math.abs(costChange).toFixed(2)}.
                        ...
                        {successRateChange && Math.abs(successRateChange) > 0.1 && (
                            <div style={{
                                ...
                                <p style={{ color: colors.text_primary, fontSize: '16px', marginBottom: '8px' }}>
                                    {successRateChange > 0 ? '✅' : '⚠️'} Success Rate {successRateChange > 0 ? 'Improved' : 'Declined'}
                                </p>
                                <p style={{ color: colors.text_secondary, fontSize: '14px' }}>
                                    Generation success rate {successRateChange > 0 ? 'improved' : 'declined'} by {Math.abs(successRateChange).toFixed(1)}%.
