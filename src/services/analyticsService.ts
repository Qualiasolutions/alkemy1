import {
  ...
  type ScriptAnalysis,
  type TechnicalPerformanceMetrics,
} from '@/types'
import { API_COST_ESTIMATES, QUALITY_THRESHOLDS } from './apiConstants'
...
export async function analyzeLookBibleAdherence(
  ...
  if (!moodboard || moodboard.length === 0) {
    return {
      score: 100,
      flaggedShots: [],
      details: { reason: 'No moodboard references to compare' },
    }
  }
...
export async function analyzeCreativeQuality(
    ...
    colorResult.flaggedShots.forEach((frameId) => {
      ...
      if (shot) {
        allFlaggedShots.push({
          ...
          severity:
            colorResult.score < 60 ? 'critical' : colorResult.score < 75 ? 'warning' : 'info',
    ...
    lightingResult.flaggedShots.forEach((frameId) => {
      ...
      if (shot && !allFlaggedShots.some((f) => f.frameId === frameId)) {
        allFlaggedShots.push({
          ...
          severity:
            lightingResult.score < 60 ? 'critical' : lightingResult.score < 75 ? 'warning' : 'info',
    ...
    lookBibleResult.flaggedShots.forEach((frameId) => {
      ...
      if (shot && !allFlaggedShots.some((f) => f.frameId === frameId)) {
        allFlaggedShots.push({
          ...
          severity:
            lookBibleResult.score < 60
              ? 'critical'
              : lookBibleResult.score < 75
                ? 'warning'
                : 'info',
  ...
  const overallColorConsistency =
    sceneReports.reduce((sum, s) => sum + s.colorConsistency, 0) / sceneReports.length
  const overallLightingCoherence =
    sceneReports.reduce((sum, s) => sum + s.lightingCoherence, 0) / sceneReports.length
  const overallLookBibleAdherence =
    sceneReports.reduce((sum, s) => sum + s.lookBibleAdherence, 0) / sceneReports.length
  const overallScore =
    (overallColorConsistency + overallLightingCoherence + overallLookBibleAdherence) / 3
...
export function trackGenerationMetrics(
  ...
  try {
    ...
    metrics.apiCosts.totalProjectCost =
      metrics.apiCosts.imageGenerationCost +
    ...
    const failedGenerations = metrics.errorRates.failedGenerations.reduce(
      (sum, e) => sum + e.count,
      0
    )
    const successfulGenerations = totalGenerations - failedGenerations
    metrics.efficiencyMetrics.successRate =
      totalGenerations > 0 ? (successfulGenerations / totalGenerations) * 100 : 100
...
export async function getPerformanceMetrics(
  projectId: string
): Promise<TechnicalPerformanceMetrics> {
...
export function getOptimizationSuggestions(
  ...
  if (imagenCost && imagenCost.count > 10 && (!fluxCost || fluxCost.count < imagenCost.count / 2)) {
    const potentialSavings =
      imagenCost.count * (API_COST_ESTIMATES.image.imagen - API_COST_ESTIMATES.image.flux)
    suggestions.push({
 ...
 */
export function detectContinuityIssues(_scriptAnalysis: ScriptAnalysis): ContinuityIssue[] {
