...
export async function generateVideoWithFal(
  ...
  if (!isVideoFalApiAvailable()) {
    throw new Error(
      'FAL API key is not configured. Please set FAL_API_KEY in environment variables.'
    )
  }
  ...
  try {
    ...
    const apiUrl =
      referenceImageUrl && modelConfig.imageToVideoUrl
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        ...
        throw new Error(
          `FAL API request error: ${errorText}. Please check your prompt and parameters.`
        )
      } else if (response.status === 404) {
        throw new Error(
          `FAL API endpoint not found (${variant}). This model may not be available or the endpoint has changed. Technical details: ${response.status} - ${errorText}`
        )
      }
    ...
    if (!result.video || !result.video.url) {
      throw new Error(
        'FAL API returned no video. This may be due to content safety filters or an invalid prompt.'
      )
    }
