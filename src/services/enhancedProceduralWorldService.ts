...
export type LightingPreset = 'golden-hour' | 'overcast' | 'neon-night' | 'studio' | 'dramatic-low-key';
...
class EnhancedProceduralWorldService {
    ...
    private fpsTracker: { frames: number; lastTime: number } = { frames: 0, lastTime: Date.now() };
    ...
    async generateEnhancedWorld(options: EnhancedWorldOptions): Promise<EnhancedProceduralWorld> {
        const { prompt, style = 'stylized', size = 'medium', complexity = 'medium', initialLightingPreset = 'golden-hour', onProgress } = options;
        ...
        try {
            ...
            throw new Error(`Failed to generate enhanced world: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    ...
    private applyLightingPreset(scene: THREE.Scene, preset: LightingPreset): void {
        ...
        scene.children.filter(obj => obj instanceof THREE.Light).forEach(light => {
            scene.remove(light);
        });
     ...
     */
    private addEnhancedControls(worldId: string, camera: THREE.PerspectiveCamera, domElement: HTMLElement): void {
    ...
    private async generateWorldStructure(prompt: string, style: string, size: string, complexity: string): Promise<any> {
...
Be creative and match the user's vision. IMPORTANT: Return ONLY the JSON object, no other text.`;
    ...
    private generateRock(scene: THREE.Scene, position: any, scale: number, rotation: number): void {
        ...
        const material = new THREE.MeshStandardMaterial({ color: '#808080', roughness: 0.9, metalness: 0.1 });
