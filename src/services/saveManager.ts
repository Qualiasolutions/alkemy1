...
class SaveManager {
  ...
  async saveNow(options: SaveOptions = {}): Promise<boolean> {
    ...
    if (!networkDetection.getStatus()) {
      const offlineError = new Error(
        'Cannot save - you appear to be offline. Changes will be saved automatically when your connection is restored.'
      )
  ...
  private async persistChanges(changes: Record<string, any>) {
    ...
    const { error } = await projectService.saveProjectData(this.currentProjectId, saveData)
  ...
  private async createVersionSnapshot(_changes: Record<string, any>) {
