...
const FLUX_API_KEY = (import.meta.env.FAL_API_KEY || import.meta.env.VITE_FAL_API_KEY || '')
  .trim()
...
export async function generateImageWithFlux(
  ...
  if (!isFluxApiAvailable()) {
    throw new Error(
      'FAL API key is not configured. Please set FAL_API_KEY or VITE_FAL_API_KEY in environment variables.'
    )
  }
  ...
  if (!modelConfig) {
    console.error(
      '[FLUX Service] Invalid model variant:',
      variant,
      'Available:',
      FLUX_MODEL_VARIANTS
    )
  ...
  try {
    ...
    if (loras && loras.length > 0) {
      ...
      console.log(
        '[FLUX Service] Using character identity LoRAs:',
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        ...
        throw new Error(
          `FAL API endpoint not found: ${modelConfig.apiUrl}. This model may not be available.`
        )
    ...
    if (!result.images || result.images.length === 0) {
      throw new Error(
        'FAL API returned no images. This may be due to content safety filters or an invalid prompt.'
      )
    }
