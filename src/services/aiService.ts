...
import { clearGeminiApiKey, getApiKeyValidationError, getGeminiApiKey } from './apiKeys'
...
import {
  ...
  getFallbackImageUrl,
  getFallbackVideoBlobs,
} from './fallbackContent'
import { type FluxModelVariant, generateImageWithFlux, isFluxApiAvailable } from './fluxService'
  ...
  setTimeout(() => {
    console.log('[AI Service] Configuration:', {
      ...
      envVars: {
        ...
        USE_FALLBACK_MODE:
          typeof process !== 'undefined' ? process.env?.USE_FALLBACK_MODE : undefined,
      },
    })
  }, 0)
}
...
function shouldUseFallbackForError(error: unknown): boolean {
  ...
  if (isAuthError) {
    console.error(
      '[AI Service] Authentication error detected - user intervention required:',
      message
    )
...
function handleApiError(error: unknown, model: string): Error {
  ...
  if (error instanceof Error) {
    ...
    if (errorLower.includes('unauthenticated') || errorLower.includes('credentials_missing')) {
      ...
      resetModelListCache()
      message =
        'Authentication failed: Your API key is invalid or in the wrong format. AI Studio keys (starting with "AQ.") cannot be used. Please generate a Google AI API key from https://aistudio.google.com/apikey'
    } else if (
      errorLower.includes('api keys are not supported') ||
      errorLower.includes('oauth2 access token')
    ) {
      window.dispatchEvent(new Event('invalid-api-key'))
      clearGeminiApiKey()
      resetModelListCache()
      message =
        'Invalid API key format: This endpoint requires a Google AI API key (starting with "AIza"), not an AI Studio key. Generate one at https://aistudio.google.com/apikey'
      ...
      message =
        'Service temporarily unavailable. The API is experiencing high load. Please wait a moment and try again.'
    } else if (errorLower.includes('safety') || errorLower.includes('prohibited_content')) {
      message =
        'Generation blocked by safety filters. The system will automatically retry with Pollinations API (free). Try simplifying your prompt or switching to a Pollinations model manually.'
...
async function image_url_to_base64(url: string): Promise<{ mimeType: string; data: string }> {
  ...
  if (url.startsWith('data:')) {
    ...
    if (!mimeMatch)
      console.warn('Could not determine MIME type from data URL, defaulting to octet-stream.', meta)
    ...
    if (!isValid) {
      ...
      throw new Error(
        `Image is too large (${sizeMB}MB). Google API supports images up to ${MAX_IMAGE_SIZE_MB}MB for inline requests. Please use a smaller image or compress it. For larger files, consider using the Files API.`
      )
    }
  ...
  try {
    ...
    if (!mimeType || !mimeType.startsWith('image/')) {
      throw new Error(
        `The content from the URL is not a valid image. MIME type found: '${mimeType || 'unknown'}'. Please use a direct link to an image file.`
      )
    }
    ...
    throw new Error(
      `Could not process a reference image from URL: ${url}. ${errorMessage}. Please ensure the URL is valid, publicly accessible, and links directly to an image.`
    )
  }
}
...
export function buildSafePrompt(
  prompt: string,
  _hasVisualReferences: boolean, // Keep for signature compatibility, though unused in this version
  _type: 'still' | 'video' = 'still',
  ...
  if (prompt.length > maxLength) {
    ...
    if (cutPoint > maxLength * 0.7) {
      // Only cut at punctuation if it's not too early
      adjustedPrompt = adjustedPrompt.substring(0, cutPoint + 1).trim()
    }
    wasAdjusted = true
    console.warn(
      `[buildSafePrompt] Prompt truncated from ${prompt.length} to ${adjustedPrompt.length} characters to prevent API errors. Context: ${isRefinement ? 'refinement' : 'generation'}`
    )
  }
...
export async function generateStillVariants(
  ...
  _avatar_refs: string[], // This argument is kept for signature compatibility but is unused.
  aspect_ratio: string,
  n: number = 1,
  moodboard?: Moodboard,
  ...
  console.log('[API Action] generateStillVariants', {
    frame_id,
    model,
    prompt,
    reference_images,
    ...
    if (result.status === 'fulfilled') {
      ...
      errors.push(
        result.value.fromFallback
          ? 'Fallback image generated – live AI service unavailable.'
          : prefersLiveGemini()
            ? null
            : 'Fallback image generated – demo mode active.'
      )
    } else {
      if (shouldUseFallbackForError(result.reason)) {
        ...
        urls.push('')
        errors.push(
          result.reason instanceof Error ? result.reason.message : 'Unknown error during generation'
        )
      }
    }
  })
...
export async function animateFrame(
  ...
  if (!prefersLiveGemini()) {
    console.warn(
      '[API Action] animateFrame using fallback videos because live service is unavailable.'
    )
    onProgress?.(100)
    const fallbackBlobs = getFallbackVideoBlobs(
      n,
      `fallback-animate-${prompt}-${reference_image_url}`
    )
  ...
  console.log('[API Action] Animating frame with Veo', {
    prompt,
  ...
  try {
    ...
    if (last_frame_image_url) {
      const { mimeType: lastMimeType, data: lastData } =
        await image_url_to_base64(last_frame_image_url)
      config.lastFrame = {
        imageBytes: lastData,
    ...
    if (downloadLinks.length === 0) {
      if (operation.error) {
        throw new Error(
          `Video generation failed: ${operation.error.message || 'Unknown Veo API error'}`
        )
      }
      throw new Error(
        'Video generation failed. The API completed without returning a video. This could be due to content safety filters.'
      )
    }

    const videoBlobs = await Promise.all(
      downloadLinks.map(async (downloadLink, _index) => {
        const apiKey = getGeminiApiKey()
        if (!apiKey) {
    ...
    if (shouldUseFallbackForError(error)) {
      ...
      const fallbackBlobs = getFallbackVideoBlobs(
        n,
        `fallback-animate-${prompt}-${reference_image_url ?? 'none'}`
      )
      if (!fallbackBlobs || !Array.isArray(fallbackBlobs)) {
...
export async function refineVariant(
  ...
  if (wasAdjusted) {
    console.warn(
      `[refineVariant] Prompt was adjusted for refinement. Original: ${prompt.length} chars, Final: ${finalPrompt.length} chars`
    )
  }
  ...
  try {
    ...
    if (errorMessage.includes('IMAGE_OTHER') || errorMessage.includes('model limitations')) {
      ...
      try {
        ...
        const { finalPrompt: retryPrompt } = buildSafePrompt(
          simplifiedPrompt,
          hasVisualReferences,
          'still',
          true
...
export async function generateVisual(
  ...
  if (
    prompt.length > MAX_SAFE_PROMPT_LENGTH &&
    (model.includes('Gemini') || model.includes('Nano Banana'))
  ) {
    console.warn(
      `[generateVisual] Prompt length (${prompt.length}) exceeds safe limit. This may cause generation failures.`
    )
    throw new Error(
  ...
  if (reference_images.length > MAX_REFERENCE_IMAGES && model.includes('Gemini')) {
    console.warn(
      `[generateVisual] Too many reference images (${reference_images.length}). Maximum is ${MAX_REFERENCE_IMAGES}.`
    )
    throw new Error(
    ...
    try {
      ...
      if (context?.projectId && context?.userId) {
        try {
          ...
          console.warn(
            '[generateVisual] Supabase upload failed, returning direct URL:',
            uploadError
          )
          return { url: imageUrl, fromFallback: false }
        }
  ...
  if (isBflModel) {
    ...
    try {
      ...
      const bflModel = model.includes('Kontext')
        ? 'flux-1-kontext'
        : model.includes('Ultra')
          ? 'flux-ultra'
          : 'flux-1-kontext' // Default

      const imageUrl = await generateImageWithBFL(prompt, bflModel, aspect_ratio, onProgress, seed)
  ...
  const isGeminiModel =
    model.includes('Gemini') || model === 'Nano Banana' || model === 'Gemini Nano Banana'
  ...
  try {
    ...
    if (effectiveModel === 'Gemini Flash Image') {
      ...
      const response = await ai.models.generateContent({
        model: 'gemini-2.0-flash-exp-image', // Alternative: gemini-2.0-flash-exp-image or gemini-1.5-flash
        contents: [
          {
            role: 'user',
            parts: parts,
    ...
    if (isSafetyError && isPollinationsAvailable() && reference_images.length === 0) {
      console.warn(
        `[generateVisual] Gemini blocked for safety. Retrying with Pollinations API (FREE) as fallback...`
      )
      try {
        ...
        console.log(
          '[generateVisual] Pollinations API fallback successful after Gemini safety block'
        )
...
export async function generateMoodboardDescription(section: MoodboardSection): Promise<string> {
  ...
  console.log('[API Action] generateMoodboardDescription', {
    notes: section.notes,
...
${
  analysis.moodboardTemplates && analysis.moodboardTemplates.length > 0
    ? analysis.moodboardTemplates
        ...
        .map(
          (board) =>
            `${board.title}: ${board.aiSummary || board.description || 'Visual references collected'}`
        )
        .join('\n')
    : analysis.moodboard
...
Remember to stay concise (2-4 sentences or tight bullet points) and redirect casual conversation back to cinematography expertise.`
    ...
    const contents = [
      {
        role: 'user',
        parts: [
          { text: `Use the provided production context to answer as the Director of Photography.` },
        ],
      },
    ...
    for (const modelId of modelAttempts) {
      try {
        ...
        if (!finalText) {
          console.warn(
            `[API Action] askTheDirector received empty response from ${modelId}, trying next candidate.`
          )
          continue
        }
    ...
    if (availabilityError) {
      console.warn(
        '[API Action] askTheDirector no preferred Gemini models available, returning fallback response.'
      )
      return fallbackDirectorResponse(analysis, query)
    }
...
export async function analyzeScript(
  ...
  if (!prefersLiveGemini()) {
    console.warn(
      '[API Action] analyzeScript using fallback parser because live service is unavailable.'
    )
  ...
  try {
    ...
    if (context?.userId && context?.projectId) {
      ...
      await logAIUsage(
        ...
        context.projectId,
        {
          ...
          totalFrames: scenesWithFrames.reduce(
            (sum, scene) => sum + (scene.frames?.length || 0),
            0
          ),
        }
      )
    ...
    return {
      ...
      moodboardTemplates: result.moodboardTemplates ?? [],
    }
  } catch (error) {
    if (shouldUseFallbackForError(error)) {
      console.warn(
        '[API Action] analyzeScript falling back to heuristic parser due to API failure.'
      )
      return fallbackScriptAnalysis(scriptContent)
    }
...
export async function generateFramesForScene(
  scene: AnalyzedScene,
  ...
  console.log(
    `[API Action] Generating frames for Scene ${scene.sceneNumber} with notes: ${directorialNotes || 'None'}`
  )
