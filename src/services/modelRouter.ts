...
export function selectBestImageModel(
  ...
  switch (strategy.budget) {
    case 'free-only':
      ...
      candidates = candidates.filter((m) => m.cost === 0 || m.freeCredits || m.monthlyFreeLimit)
      break
...
export function selectBestVideoModel(
  ...
  switch (strategy.budget) {
    case 'free-only':
      candidates = candidates.filter((m) => m.cost === 0 || m.monthlyFreeLimit)
      break
...
export function getFallbackModels(model: string, maxFallbacks: number = 3): string[] {
  ...
  const fallbacks = IMAGE_MODELS.filter(
    (m) =>
      m.model !== model &&
...
export function estimateProjectCost(
  ...
  const videoModel = selectBestVideoModel({ needsHighQuality: false, duration: 5 }, strategy)
  ...
  const maxVideoCost = 0.6 * videoCount // Veo 3.1 max
