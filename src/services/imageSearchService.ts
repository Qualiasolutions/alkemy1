...
const BRAVE_PROXY_URL =
  (importMetaEnv.VITE_BRAVE_PROXY_URL as string | undefined) || '/api/brave-proxy'
// Always use proxy in browser environment for security (API key should never be exposed client-side)
const _shouldUseBraveProxy = true
...
["query1", "query2", "query3", ...]`
    ...
    for (let i = 0; i < Math.min(searchQueries.length, 3); i++) {
      ...
      try {
        ...
        if (!response || !response.ok) {
          console.warn(
            `Brave Search API error for query "${query}": ${response?.status ?? 'network'} ${response?.statusText ?? 'failed request'}`
          )
          continue
        }
        ...
        try {
          ...
          console.error(
            `Brave proxy response was not valid JSON for "${query}":`,
            parseError,
            rawBody.slice(0, 200)
          )
          continue
    ...
    const validImages = images.filter((img) => img.url?.startsWith('http')).slice(0, 100) // Limit to 100 results
...
async function fetchBraveResponse(
  ...
  try {
    ...
    if (!response.ok) {
      console.error(
        `Brave proxy request failed with status ${response.status} for query: "${query}"`
      )
 ...
 */
function buildCinematographyPrompt(category: string, specs: any): string {
...
Provide 5 search queries to find similar images.`
    ...
    throw new Error(
      `Failed to search similar images: ${error instanceof Error ? error.message : 'Unknown error'}`
    )
  }
}
...
async function searchFrameSet(query: string, count: number): Promise<SearchedImage[]> {
  try {
    ...
    const titleMatches = html.match(
      /<[^>]*>([^<]*(?:film|movie|scene|frame|shot|cinema)[^<]*)<[^>]*>/gi
    )
