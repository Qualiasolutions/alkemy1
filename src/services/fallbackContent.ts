...
function createMoodboardSection(
  seed: string,
  notes: string,
  aspectRatio: string
): MoodboardSection {
...
const defaultMoodboard = (title: string): Moodboard => ({
  cinematography: createMoodboardSection(
    `${title}-cine`,
    'Fallback cinematography palette blending moody interiors and dramatic lighting.',
    '16:9'
  ),
  color: createMoodboardSection(
    `${title}-color`,
    'Rich contrast between warm highlights and cooler shadows.',
    '1:1'
  ),
  style: createMoodboardSection(
    `${title}-style`,
    'Tailored, modern styling with hints of noir-inspired silhouettes.',
    '3:4'
  ),
  other: createMoodboardSection(
    `${title}-other`,
    'Supporting textures and atmospheric references for tone.',
    '16:9'
  ),
})
  ...
  return [
    {
      ...
      items: aggregated,
      aiSummary:
        'A quick offline palette blending cinematography, color, styling, and texture references.',
...
export const fallbackMoodboardDescription = (section: MoodboardSection): string => {
  const notePortion = section.notes
    ? `Notes highlight: ${section.notes}.`
    : 'No user notes were provided for this section.'
  const itemCount = section.items.length
  const imagery =
    itemCount > 0
      ? `Using ${itemCount} visual references, lean into cohesive tones and textures to maintain continuity.`
      : 'Add images to enrich this section when possible.'
...
export const fallbackScriptAnalysis = (scriptContent: string): ScriptAnalysis => {
  ...
  const summaryParagraphs = trimmed
    .split(/\n\s*\n/)
    .map((p) => sanitizeText(p))
    .filter(Boolean)
  const summary =
    summaryParagraphs.slice(0, 2).join(' ') ||
    'Fallback summary generated automatically. Refine once AI services are restored.'
