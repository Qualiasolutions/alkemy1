...
import type { ContinuityIssue, ScriptAnalysis, TimelineClip } from '../types'
...
export async function analyzeContinuity(
  ...
  for (let i = 0; i < timelineClips.length - 1; i++) {
    ...
    if (
      clip1.sceneNumber !== null &&
      clip2.sceneNumber !== null &&
      clip1.sceneNumber !== clip2.sceneNumber
    ) {
      continue
    }
...
export async function detectLightingJumps(
  ...
  const _hasLighting1 = lightingKeywords.some((kw) => desc1.includes(kw))
  const _hasLighting2 = lightingKeywords.some((kw) => desc2.includes(kw))

  // Detect conflicting lighting descriptions
  const isDarkToLight =
    (desc1.includes('dark') || desc1.includes('night')) &&
    (desc2.includes('bright') || desc2.includes('day') || desc2.includes('sunny'))
  const isLightToDark =
    (desc1.includes('bright') || desc1.includes('day') || desc1.includes('sunny')) &&
    (desc2.includes('dark') || desc2.includes('night'))
...
export async function detectCostumeChanges(
  clip1: TimelineClip,
  clip2: TimelineClip,
  _scriptAnalysis: ScriptAnalysis | null
): Promise<ContinuityIssue | null> {
...
export async function detectSpatialMismatches(
  ...
  const exitsLeft1 =
    desc1.includes('exits left') || desc1.includes('walks left') || desc1.includes('moves left')
  const exitsRight1 =
    desc1.includes('exits right') || desc1.includes('walks right') || desc1.includes('moves right')
 ...
 */
export function dismissWarning(issueId: string, _reason: string): void {
