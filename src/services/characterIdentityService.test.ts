...
const mockGenerateImageWithPollinations = vi
  .fn()
...
global.fetch = vi.fn().mockImplementation((urlOrRequest) => {
  ...
  return Promise.resolve({
    ...
    blob: vi
      .fn()
      .mockResolvedValue(new Blob(['default-data'], { type: 'application/octet-stream' })),
  })
})
...
describe('Character Identity Service - Story 2.2', () => {
  ...
  describe('generateAllTests - AC2 (Batch Testing)', () => {
    it('should generate all 5 test types', async () => {
      ...
      expect(tests.map((t) => t.testType)).toEqual([
        'portrait',
        'fullbody',
        'profile',
        'lighting',
  ...
  describe('approveCharacterIdentity - AC5', () => {
    ...
    it('should preserve all original identity properties', async () => {
      ...
      expect(approvedIdentity.trainingCost).toBe(0.1)
  ...
  describe('Test Type Prompts', () => {
    it('should use appropriate prompts for each test type', async () => {
      ...
      const portraitCall = (global.fetch as any).mock.calls[
        (global.fetch as any).mock.calls.length - 1
      ]
      if (portraitCall?.[1]?.body) {
      ...
      const lightingCall = (global.fetch as any).mock.calls[
        (global.fetch as any).mock.calls.length - 1
      ]
      if (lightingCall?.[1]?.body) {
