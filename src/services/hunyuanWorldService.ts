...
class HunyuanWorldService {
      ...
      try {
        ...
        const generationPromise = new Promise<any>(async (resolve, reject) => {
          ...
          timeoutHandle = setTimeout(() => {
            ...
            reject(
              new Error('Generation timeout - GPU queue is overloaded. Please try again later.')
            )
          }, timeoutMs)
          ...
          heartbeatInterval = setInterval(() => {
            ...
            if (timeSinceLastMessage > 600000) {
              // 10 minutes without updates
              if (heartbeatInterval) clearInterval(heartbeatInterval)
            ...
            for await (const message of job) {
              ...
              if (message.type === 'status') {
                if (message.stage === 'pending') {
                  const queueInfo = message.queue_position
                    ? ` (position ${message.queue_position} in queue)`
                    : ''
                  const estimatedTime = message.queue_position
                    ? ` (~${Math.ceil(message.queue_position * 2)} min estimated)`
                    : ''
        ...
        if (isLastAttempt) {
          throw new Error(
            `${errorMessage}. The HunyuanWorld Space is currently overloaded. Try again in a few minutes or use a different quality setting.`
          )
        }
        ...
        onProgress?.(
          15,
          `GPU busy - retrying in ${waitTime / 1000}s... (attempt ${attempt + 1}/${maxRetries})`
        )
  ...
  async generateWorld(options: HunyuanWorldOptions): Promise<HunyuanWorldResult> {
    ...
    try {
      ...
      const [shapeFile, textureFile, _htmlOutput, meshStats, resultSeed] = result.data
      ...
      if (format !== 'glb') {
        ...
        const [_exportHtml, exportedFile] = exportResult.data
  ...
  private async downloadFile(fileUrlOrPath: string | { url: string }): Promise<Blob> {
    ...
    const fullUrl = url.startsWith('http')
      ? url
      : `https://tencent-hunyuan3d-2.hf.space/file=${url}`
