...
import type { Project, RoadmapBlock, ScriptAnalysis, TimelineClip } from '../types'

const PROJECT_STORAGE_KEY = 'alkemy_ai_studio_project_data_v2'

const _isSupabaseProjectId = (id?: string | null): boolean => {
  if (!id) return false
...
const _createEmptyScriptAnalysis = (): ScriptAnalysis => ({
...
export const useProjectState = (user: any): UseProjectStateReturn => {
  ...
  const initializeProject = useCallback(
    async (user: any, searchParams: URLSearchParams) => {
      setIsLoadingProjects(true)
  ...
  const handleNewProject = useCallback(async () => {
    ...
    try {
      const projectService = getProjectService()
      const newProject = await projectService.createProject('Untitled Project', user.id)
  ...
  }, [hasLoadedInitialProject, getSerializableState])
