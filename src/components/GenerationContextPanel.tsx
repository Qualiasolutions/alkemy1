...
import type {
  ...
  GenerationContext,
  LocationWith3DWorld,
} from '@/services/generationContext'
import {
  CheckCircleIcon,
  ChevronDownIcon,
  ImageIcon,
...
const GenerationContextPanel: React.FC<GenerationContextPanelProps> = ({
  ...
  const hasAllIdentities =
    selectedCharacters.length > 0 && identityCount === selectedCharacters.length
    ...
    >
      ...
      <div className="p-5 space-y-4">
        ...
        <div className="space-y-3">
          ...
          {selectedLocation ? (
            <LocationCard location={selectedLocation} onRemove={() => onLocationChange('')} />
          ) : (
            <div className="text-center py-6 border-2 border-dashed border-gray-700/50 rounded-xl">
