...
import { CameraMovementType, MotionType, type Point2D } from '../services/ttmService'
...
export default function MotionTrajectoryEditor({
  ...
  useEffect(() => {
    ...
    img.src = imageUrl
  }, [imageUrl, trajectory, previewTrajectory, drawOverlay, drawTrajectory])
  ...
  const drawTrajectory = useCallback(
    (ctx: CanvasRenderingContext2D, points: Point2D[], isPreview = false) => {
      ...
      ctx.save()
      ctx.strokeStyle = isPreview ? 'rgba(255, 255, 255, 0.3)' : 'rgba(255, 100, 100, 0.8)'
      ...
      ctx.fillStyle = isPreview ? 'rgba(255, 255, 255, 0.5)' : 'rgba(255, 100, 100, 1)'

      // Sample points for drawing (max 50 points for performance)
      const pointsForDrawing =
        pointsToDraw.length > 50
          ...
          <div className="flex items-center justify-center space-x-4">
            ...
            <div className="flex flex-col items-center">
              ...
              <input
                ...
                onChange={(e) =>
                  handleCameraControl(CameraMovementType.ZOOM, parseFloat(e.target.value))
                }
                className="w-32"
              />
            ...
            <div className="flex flex-col items-center">
              ...
              <input
                ...
                onChange={(e) =>
                  handleCameraControl(CameraMovementType.ORBIT, parseFloat(e.target.value))
                }
                className="w-32"
              />
              <span className="text-xs text-gray-500 mt-1">
