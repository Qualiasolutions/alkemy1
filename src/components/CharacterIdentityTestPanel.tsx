...
import type { AnalyzedCharacter, CharacterIdentityTest, CharacterIdentityTestType } from '../types'
...
export function CharacterIdentityTestPanel({
  ...
  const averageScore =
    tests.length > 0 ? tests.reduce((sum, test) => sum + test.similarityScore, 0) / tests.length : 0
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <h3
          className={`text-xl font-bold mb-2 flex items-center gap-2 ${isDark ? 'text-white' : 'text-gray-900'}`}
        >
        ...
        {tests.length > 0 && (
          ...
          >
            ...
            <div
              className={`px-3 py-1 rounded-full text-sm font-bold border ${getScoreBadgeClass(averageScore)}`}
            >
          ...
          {(Object.keys(TEST_TYPE_CONFIG) as CharacterIdentityTestType[]).map((testType) => {
            ...
            return (
              ...
              >
                ...
                {existingTest && (
                  <div
                    className={`text-xs px-2 py-1 rounded border inline-block ${getScoreBadgeClass(existingTest.similarityScore)}`}
                  >
                    {existingTest.similarityScore.toFixed(1)}% -{' '}
                    {getScoreLabel(existingTest.similarityScore)}
                  </div>
                )}
              </button>
            )
            ...
            {tests.map((test) => (
              ...
              >
                ...
                <div className="p-2 space-y-1">
                  ...
                  <div
                    className={`text-xs px-2 py-0.5 rounded text-center border ${getScoreBadgeClass(test.similarityScore)}`}
                  >
      ...
      {selectedTest && (
        ...
        >
          ...
          >
            ...
            <div
              className={`px-3 py-1 rounded border ${getScoreBadgeClass(selectedTest.similarityScore)}`}
            >
          ...
          >
            <p>
              <strong>Test Type:</strong> {TEST_TYPE_CONFIG[selectedTest.testType].label}
            </p>
            <p>
              <strong>Description:</strong> {TEST_TYPE_CONFIG[selectedTest.testType].description}
            </p>
            <p>
              <strong>Generated:</strong> {new Date(selectedTest.timestamp).toLocaleString()}
            </p>
            <p>
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs mt-1">
                  Average score is below 85%. Consider uploading different reference images or
                  adjusting training parameters for better results.
                </p>
              </div>
            </div>
          )}
