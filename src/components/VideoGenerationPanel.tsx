...
import { generateTextToVideoWithHF, generateVideoWithHF } from '../services/huggingFaceService'
import {
...
import type { AnalyzedCharacter, AnalyzedLocation, Moodboard, MoodboardTemplate } from '../types'
import {
  DownloadIcon,
  FileVideoIcon,
  PaperclipIcon,
...
const VideoGenerationPanel: React.FC<VideoGenerationPanelProps> = ({
  ...
  const [_selectedVideo, _setSelectedVideo] = useState<VideoGeneration | null>(null)
  ...
  const handleGenerateVideo = async () => {
    ...
    if (
      ...
      selectedModel === 'Veo 2' ||
      selectedModel === 'SVD (Free)'
    ) {
      const referenceImage = attachedImage || item.data.imageUrl
      if (!referenceImage) {
        alert(
          `${selectedModel} requires a reference image. Please attach an image or ensure the item has one.`
        )
        return
      }
    }
    ...
    try {
      ...
      const onProgress = (progress: number) => {
        setVideoGenerations((prev) =>
          prev.map((v) => (v.id === newVideoId ? { ...v, progress } : v))
        )
      }
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-shrink-0 p-5 border-b-2 border-purple-500/20 bg-gradient-to-r from-purple-900/20 to-pink-900/20">
        ...
        <p className="text-xs text-gray-400 mt-1">Create dynamic videos for {item.data.name}</p>
      </div>
            ...
            <motion.div
              ...
              className={`relative rounded-xl overflow-hidden border-2 transition-all ${
                video.status === 'generating'
                  ? 'border-#DFEC2D/30 bg-yellow-900/10'
                  : video.status === 'error'
            ...
            <div className="flex gap-2 flex-wrap">
              {(
                ...
                >
                  ...
                  {model.includes('Free') && <span className="ml-1 text-green-400">✨</span>}
                </button>
              ))}
            </div>
          </div>
