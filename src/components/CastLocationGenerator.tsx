...
import { generateStillVariants, refineVariant } from '../services/aiService'
import { getCharacterIdentityStatus } from '../services/characterIdentityService'
import { useTheme } from '../theme/ThemeContext'
import type { AnalyzedCharacter, AnalyzedLocation, Generation, Moodboard } from '../types'
import {
  AlkemyLoadingIcon,
  ArrowLeftIcon,
  CheckIcon,
...
const RefineStudio: React.FC<{
  ...
  const handleRefine = async () => {
    ...
    try {
      const refinedImageUrl = await refineVariant(prompt, currentImage, aspectRatio, {
  ...
  const handleSetMainAndClose = () => {
    if (currentImage) {
      onUpdateItem((prev) => {
        ...
        const updatedGenerations = hasImage
          ? prev.generations
          : [
              ...(prev.generations || []),
              {
...
const CastLocationGenerator: React.FC<CastLocationGeneratorProps> = ({
  ...
  const [generationCount, _setGenerationCount] = useState(1)
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (attachedImages.length > 0) {
        ...
        console.log(
          '[CastLocationGenerator] Using attached reference images:',
      ...
      const selectedMoodboardTemplate =
      ...
      const effectiveMoodboard = selectedMoodboardTemplate
        ? {
            cinematography: {
              notes: selectedMoodboardTemplate.title,
              items: selectedMoodboardTemplate.items,
              aiDescription: selectedMoodboardTemplate.aiSummary,
            },
    ...
    <div className="fixed inset-0 flex bg-[#0a0a0a] overflow-hidden z-40">
      ...
      <div className="w-72 bg-gradient-to-b from-white/5 to-white/[0.02] backdrop-blur-xl border-r border-white/10 flex flex-col p-5 gap-3 h-full overflow-y-auto">
        ...
        <div>
          ...
          >
            <option value="none" className="bg-[#0a0a0a]">
              No Moodboard (Default)
            </option>
        ...
        {item.data.imageUrl && (
          <div>
            ...
            >
              <div
                className={`${aspectRatioClasses[aspectRatio] || 'aspect-[4/3]'} relative overflow-hidden rounded-lg`}
              >
        ...
        <div>
          ...
          <div className="space-y-2">
            ...
            {item.data.identity?.referenceImages &&
              item.data.identity.referenceImages.length > 0 && (
                <div className="grid grid-cols-3 gap-2">
                  {item.data.identity.referenceImages.slice(0, 6).map((imgUrl, idx) => (
                    <div
                      key={idx}
                      className="relative group aspect-square rounded-lg overflow-hidden border border-[#DFEC2D]/30"
                    >
                      <img
                        src={imgUrl}
                        alt={`Reference ${idx + 1}`}
            ...
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-1.5">
                ...
                <div className="group relative">
                  <svg
                    className="w-3 h-3 text-white/30 cursor-help"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                  ...
                  <div className="hidden group-hover:block absolute bottom-full left-0 mb-2 w-64 p-3 bg-black/90 border border-white/20 rounded-lg text-[10px] text-white/80 z-50">
                    ...
                    <p>
                      Train a LoRA (Low-Rank Adaptation) model on 3-5 images of your character. This
                      ensures consistent appearance across all generated shots with 90-98% visual
                      similarity.
                    </p>
                  </div>
                </div>
              ...
              {getCharacterIdentityStatus(character.identity) === 'ready' && (
                <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-[#DFEC2D]/20 border border-[#DFEC2D]/30">
                  ...
                  <span className="text-[10px] font-semibold text-[#DFEC2D] uppercase tracking-wider">
                    LoRA Active
                  </span>
                </div>
              )}
            ...
            <div className="flex items-center gap-2 mb-3">
              <div
                className={`w-2 h-2 rounded-full ${
                  getCharacterIdentityStatus(character.identity) === 'ready'
                    ? 'bg-[#DFEC2D]/100'
                    : getCharacterIdentityStatus(character.identity) === 'preparing'
                      ? 'bg-[#DFEC2D] animate-pulse'
                      : getCharacterIdentityStatus(character.identity) === 'error'
                        ? 'bg-red-500'
                        : 'bg-gray-500'
                }`}
              ></div>
              ...
              <div className="mt-4 pt-4 border-t border-white/10">
                  ...
                  <div
                    className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${
                      loraImages.length >= 6 && loraImages.length <= 12
                        ...
                        : 'bg-[#DFEC2D]/20 text-yellow-900 border border-[#DFEC2D]/30'
                    }`}
                  >
                ...
                <div className="grid grid-cols-3 gap-2 max-h-48 overflow-y-auto">
                  {loraImages.map((imgUrl, idx) => (
                    <div
                      key={idx}
                      className="relative group aspect-square rounded-lg overflow-hidden border border-purple-500/30"
                    >
                      <img
                        src={imgUrl}
                        alt={`LoRA ${idx + 1}`}
        ...
        <div className="p-4 md:p-6">
          <div className="w-full">
            ...
            {isGenerating &&
              ...
              allGenerations.filter((g) => g.isLoading).length === generationCount && (
                <div className="flex flex-col items-center justify-center h-96 text-center">
                  ...
                  <p className="text-white/60">
                    Preparing to generate {generationCount}{' '}
                    {generationCount === 1 ? 'image' : 'images'}
                  </p>
                </div>
              )}
            ...
            {validGenerations.length > 0 ? (
                          ...
                          <div className="absolute bottom-2 left-2 right-2 flex gap-2 justify-center">
                            {isCharacter ? (
                              <button
                                ...
                                title={
                                  loraImages.includes(gen.url!) ? 'Remove from LoRA' : 'Add to LoRA'
                                }
                              >
                                {loraImages.includes(gen.url!) ? (
                            ...
                            <button
                              onClick={(e) => {
                                ...
                                handleDownloadImage(
                                  gen.url!,
                                  `${item.type}-${item.data.name.replace(/[^a-zA-Z0-9]/g, '_')}-gen-${idx + 1}.png`
                                )
                              }}
              ...
              <div className="flex flex-col items-center justify-center h-96 text-center">
                ...
                <p className="text-white/60 mb-6">
                  Enter a prompt and click Generate to create images for {item.data.name}
                </p>
                <div className="text-sm text-white/40">
                  {attachedImages.length > 0 ? (
                    <span>
                      ✓ {attachedImages.length} reference image
                      {attachedImages.length > 1 ? 's' : ''} attached
                    </span>
                  ) : (
                    <span>
                      💡 Tip: Attach reference images for better results{' '}
                      {isFluxMulti ? '(Multiple supported)' : '(Single image)'}
                    </span>
                  )}
                </div>
              </div>
          ...
          >
            ...
            <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 bg-black/50 backdrop-blur-sm px-4 py-2 rounded-full">
              <p className="text-white text-sm">
                ...
                {[item.data.imageUrl ? 1 : 0, ...validGenerations].length} • Use arrow keys to
                navigate • Press 'D' to download
              </p>
            </div>
            ...
            <button
              onClick={() => {
                const allImages = [
                  ...(item.data.imageUrl
                    ? [{ id: 'main', url: item.data.imageUrl } as Generation]
                    : []),
                  ...validGenerations,
                ]
                const currentIndex = allImages.findIndex((img) => img.url === viewingGeneration.url)
                  ...
                  <>
                    ...
                    {isCharacter ? (
                      <button
                        ...
                        className={`px-4 py-2 rounded-lg transition flex items-center gap-2 ${
                          viewingGeneration.url && loraImages.includes(viewingGeneration.url)
                            ...
                            : 'bg-[#DFEC2D] hover:bg-[#FDE047] text-white'
                        }`}
                      >
                        {viewingGeneration.url && loraImages.includes(viewingGeneration.url) ? (
                          <>
                      ...
                      <button
                        ...
                        className="px-4 py-2 bg-[#DFEC2D] text-white rounded-lg hover:bg-[#FDE047] transition"
                      >
                        Set as Main
                      </button>
                    )}
