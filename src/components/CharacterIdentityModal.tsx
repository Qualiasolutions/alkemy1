...
import { prepareCharacterIdentity } from '@/services/characterIdentityService'
import { useTheme } from '@/theme/ThemeContext'
import type { CharacterIdentity } from '@/types'
import Button from './Button'
import { AlertCircleIcon, CheckCircleIcon, Trash2Icon, UploadIcon, XIcon } from './icons/Icons'
...
const CharacterIdentityModal: React.FC<CharacterIdentityModalProps> = ({
    ...
    const loadInitialImages = async () => {
      ...
      for (let i = 0; i < initialImages.length; i++) {
        ...
        try {
          ...
          const file = new File([blob], `lora-image-${i + 1}.png`, {
  ...
  const validateImageFile = async (
    ...
    if (file.size > maxSize) {
      return {
        isValid: false,
        error: `File size exceeds 10MB (${(file.size / 1024 / 1024).toFixed(2)}MB)`,
      }
    }
  ...
  const handlePrepareIdentity = async () => {
    ...
    if (images.length < 6) {
      setError(
        `At least 6 reference images are required for optimal training. You have ${images.length}. Upload ${6 - images.length} more.`
      )
      return
    }
    ...
    if (invalidImages.length > 0) {
      setError(
        `${invalidImages.length} image(s) have validation errors. Please remove or replace them.`
      )
      return
    }
    ...
    >
      ...
      >
        ...
        >
          <div>
            <h3
              id="character-identity-modal-title"
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
                ...
                {images.map((img, index) => (
                  <motion.div
                    ...
                    className={`relative group rounded-lg overflow-hidden aspect-square border-2 ${
                      img.isValid ? 'border-[#DFEC2D]/30' : 'border-red-500/50'
                    }`}
                  >
                    <img
                      src={img.preview}
                    ...
                    <div
                      className={`absolute top-1 left-1 p-1 rounded-full ${
                        img.isValid ? 'bg-[#DFEC2D]/100/90' : 'bg-red-500/90'
                      }`}
                    >
                      {img.isValid ? (
            ...
            >
              <p
                className={`font-semibold mb-1 ${isDark ? 'text-[var(--color-accent-primary)]' : 'text-[var(--color-accent-primary)]'}`}
              >
                💡 Tips for best results:
              </p>
              <ul
                className={`space-y-0.5 ml-4 list-disc ${isDark ? 'text-[var(--color-accent-primary)]' : 'text-[var(--color-accent-primary)]'}`}
              >
