...
import { ThemeProvider } from './theme/ThemeContextSimple';
...
import { TABS, TABS_CONFIG } from './constants/index';
...
// import GenerateWorldTab from './tabs/GenerateWorldTab'; // Removed - feature not yet implemented
    ...
    <div className={`flex items-center justify-center h-screen ${isDark ? 'bg-[#0B0B0B]' : 'bg-[#FFFFFF]'}`}>
      ...
      >
        ...
        <div className="space-y-3">
          <Button onClick={handleSelectKey} variant="default" className="w-full !py-3 !text-base" disabled={isSelecting}>
...
const PlaceholderTab: React.FC<{ tabName: string; message?: string }> = ({ tabName, message }) => (
  <div className="flex flex-col items-center justify-center h-full text-gray-500">
    <h2 className="text-2xl font-bold mb-2">{tabName}</h2>
...
const AppContentBase: React.FC<AppContentBaseProps> = ({ user, isAuthenticated, authLoading }) => {
  ...
  const [saveDialogMode, setSaveDialogMode] = useState<'save' | 'save-as'>('save');
  ...
  const handleOpenSaveDialog = useCallback((mode: 'save' | 'save-as' = 'save') => {
    if (!currentProject) {
  ...
  // Handler for save dialog completion
  const handleSaveDialogComplete = useCallback((updatedProject: Project) => {
    setCurrentProject(updatedProject);
    showToast(`Project ${saveDialogMode === 'save' ? 'saved' : 'copied'} successfully!`);
  }, [saveDialogMode, showToast]);
  ...
  const handleDeleteComplete = useCallback(async (projectIds: string[], permanent: boolean) => {
    if (currentProject && projectIds.includes(currentProject.id)) {
      // Current project was deleted, create a new one
      await handleNewProject();
      showToast(`Project ${permanent ? 'permanently deleted' : 'moved to trash'}!`);
    }
  }, [currentProject, handleNewProject, showToast]);
        ...
        <div className="text-center p-8 max-w-md">
          ...
          <div className="space-y-3">
            <button
              ...
              className="w-full px-4 py-2 bg-[#DFEC2D] text-white rounded-lg hover:bg-[#FDE047] transition-colors"
            >
              Reload Application
            </button>
            <button
  ...
  return (
    <div className="relative flex min-h-screen overflow-hidden bg-[#0B0B0B]">
      {/* Subtle background accents */}
      <div className="pointer-events-none absolute -top-32 left-1/2 h-[520px] w-[520px] -translate-x-1/2 rounded-full bg-yellow-500/5 blur-3xl" />
      <div className="pointer-events-none absolute bottom-0 right-[-10%] h-[420px] w-[420px] rounded-full bg-yellow-500/3 blur-3xl" />
      ...
      <div className="relative flex flex-1 flex-col">
        <motion.header
          ...
          className="sticky top-0 z-30 border-b border-zinc-800 bg-black/95 backdrop-blur-xl text-white shadow-lg shadow-black/20"
        >
            ...
            <div className="flex items-center gap-6">
              <div className="flex flex-col gap-1">
                <div className="flex items-center gap-3">
                  <h1 className="text-2xl font-semibold tracking-tight text-white">{activeTabMeta?.name ?? 'Alkemy AI Studio'}</h1>
                  {scriptAnalysis?.title && (
                    <span className="rounded-full px-3 py-1 text-xs font-medium border border-zinc-700 bg-zinc-900/50 text-zinc-300">
                      {scriptAnalysis.title}
                    </span>
                  )}
                </div>
                <p className="text-xs uppercase tracking-[0.4em] text-zinc-400">{activePhase}</p>
              </div>
            </div>

            {/* Center Section - Project Management Controls */}
            <div className="flex items-center gap-3">
              {/* Project Name Indicator */}
              <div className="flex items-center gap-2 rounded-full px-4 py-2 bg-zinc-900/50 text-zinc-300 border border-zinc-700">
                <motion.span
                  animate={{ scale: [1, 1.25, 1], opacity: [0.8, 1, 0.8] }}
                  transition={{ duration: 2, repeat: Infinity, ease: 'easeInOut' }}
                  className="inline-flex h-2 w-2 rounded-full bg-yellow-500 shadow-[0_0_12px_rgba(250,204,21,0.8)]"
                />
                <span className="text-xs font-medium uppercase tracking-[0.3em]">
                  {currentProject?.title || scriptAnalysis?.title || 'Untitled Project'}
                </span>
              </div>

              {/* Projects Button - Prominently placed */}
              {isAuthenticated && (
                <motion.button
                  onClick={() => setShowProjectSelector(true)}
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.96 }}
                  className="flex items-center gap-2 rounded-full px-5 py-2.5 text-sm font-medium transition-all border border-yellow-500/50 bg-yellow-500/10 text-yellow-400 hover:bg-yellow-500/20 hover:border-yellow-400 hover:shadow-lg hover:shadow-yellow-500/15"
              ...
              <motion.button
                ...
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.96 }}
                className="flex items-center gap-2 rounded-full px-5 py-2.5 text-sm font-medium transition-all border border-zinc-700 bg-zinc-900/50 text-zinc-300 hover:border-zinc-600 hover:text-white hover:shadow-lg hover:shadow-zinc-700/20"
              ...
              {/* Save and Delete buttons - only show when authenticated and project exists */}
              {isAuthenticated && currentProject && (
                <>
                  <motion.button
                    ...
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.96 }}
                    className="flex items-center gap-2 rounded-full px-5 py-2.5 text-sm font-medium transition-all border border-zinc-700 bg-zinc-900/50 text-zinc-300 hover:border-yellow-500/50 hover:text-yellow-400 hover:shadow-lg hover:shadow-yellow-500/20"
                    title="Save Project"
                  >
                    <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <motion.button
                    ...
                    whileHover={{ scale: 1.05 }}
                    whileTap={{ scale: 0.96 }}
                    className="flex items-center gap-2 rounded-full px-5 py-2.5 text-sm font-medium transition-all border border-zinc-700 bg-zinc-900/50 text-zinc-300 hover:border-red-500/50 hover:text-red-400 hover:shadow-lg hover:shadow-red-500/20"
      ...
      {/* Project Selector Modal - Enhanced with trash and templates */}
