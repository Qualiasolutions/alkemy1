...
const _PresentationTab = React.lazy(() => import('./tabs/PresentationTab'))
...
const _FramesTab = React.lazy(() => import('./tabs/FramesTab.simple'))
const _WanTransferTab = React.lazy(() => import('./tabs/WanTransferTab'))
const ExportsTab = React.lazy(() => import('./tabs/ExportsTab'))
const RoadmapTab = React.lazy(() => import('./tabs/RoadmapTab'))
const _ProjectRoadmapTab = React.lazy(() => import('./tabs/ProjectRoadmapTab'))
const ThreeDWorldsTab = React.lazy(() => import('./tabs/3DWorldsTab'))
const _GenerateTab = React.lazy(() => import('./tabs/GenerateTab'))
...
const _PROJECT_STORAGE_KEY = 'alkemy_ai_studio_project_data_v2'

// API Key Component
const ApiKeyPrompt: React.FC<{ onKeySelected: () => void; isDark?: boolean }> = React.memo(
  ({ onKeySelected, isDark = true }) => {
    ...
    const handleSelectKey = useCallback(async () => {
      if (!window.aistudio?.openSelectKey) {
        setError(
          'API key selection utility is not available in this environment. Paste your key below instead.'
        )
        return
      }
        ...
        <div className="prompt-content">
          ...
          {window.aistudio?.openSelectKey && (
            <div className="key-select-section">
              <button onClick={handleSelectKey} disabled={isSelecting} className="select-button">
...
const AppContent: React.FC = React.memo(() => {
  ...
  const [_isCheckingKey, setIsCheckingKey] = useState<boolean>(() => !initialHasGeminiKey)
  const [showAuthModal, setShowAuthModal] = useState<boolean>(false)
  const [authModalMode, setAuthModalMode] = useState<'login' | 'register'>('login')
  const [showProjectSelector, setShowProjectSelector] = useState<boolean>(false)
  const [autoProjectCreated, setAutoProjectCreated] = useState<boolean>(false)
  const [preferencesError, _setPreferencesError] = useState<string | null>(null)
  ...
  const _supabaseEnabled = useMemo(() => isSupabaseConfigured(), [])
  ...
  useEffect(() => {
    ...
    isAuthenticated,
    user,
    projectState.hasLoadedInitialProject,
    searchParams,
    projectState.initializeProject,
  ])
  ...
  useEffect(() => {
    if (isAuthenticated && user && !autoProjectCreated && projectState.projectList.length === 0) {
      const _createDemoProject = async () => {
        try {
          const projectService = getProjectService()
          const demoProject = await projectService.createProject('Demo Project', user.id)
  ...
  const renderContent = useMemo(() => {
    ...
    if (showSplash) {
      return <SplashScreen onLoadComplete={() => setShowSplash(false)} isDark={isDark} />
    }
    ...
    if (!authLoading && !isAuthenticated) {
      return <WelcomeScreen onAuthClick={() => setShowAuthModal(true)} isDark={isDark} />
    }
