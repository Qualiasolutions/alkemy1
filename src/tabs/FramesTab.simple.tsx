...
const FramesTab: React.FC<FramesTabProps> = ({
  ...
  const _SNAP_THRESHOLD = 0.1 // seconds
  ...
  const getClipAtTime = useCallback(
    (time: number): { clip: TimelineClip; localTime: number; clipIndex: number } | null => {
    ...
    const handleTimeUpdate = () => {
      ...
      if (videoElement.currentTime >= clip.trimEnd) {
        ...
        if (nextClipIndex < clips.length) {
          const _nextClip = clips[nextClipIndex]
          ...
          if (clip.timelineId === isDraggingTrim.clipId) {
            if (isDraggingTrim.handle === 'start') {
              ...
              const newTrimEnd = Math.min(
                clip.sourceDuration,
                Math.max(dragTime, clip.trimStart + 0.1)
              )
                ...
                {currentPlayingClip ? (
                  <>
                    <p className="text-xs text-zinc-400 mb-1">
                      Scene {currentPlayingClip.clip.sceneNumber} - Shot{' '}
                      {currentPlayingClip.clip.shot_number}
                    </p>
              ...
              <div className="relative" style={{ height: TRACK_HEIGHT + 60 }}>
                ...
                <div className="absolute top-0 left-0 right-0 h-6 border-b border-zinc-700 flex items-end">
                  {Array.from({ length: Math.ceil(totalDuration) + 1 }).map((_, i) => (
                    <div
                      key={i}
                      style={{ position: 'absolute', left: i * PIXELS_PER_SECOND }}
                        ...
                        >
                          ...
                          <div className="absolute inset-0 flex items-center px-2 bg-gradient-to-t from-black/80 via-black/40 to-transparent">
                            <div className="flex-1 truncate">
                              <p className="text-xs font-semibold truncate drop-shadow-md">
                                {clip.description}
                              </p>
