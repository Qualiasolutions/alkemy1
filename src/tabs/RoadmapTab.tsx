...
import { PlusIcon, TrashIcon, LinkIcon, DownloadIcon, UploadIcon, ZoomInIcon, ZoomOutIcon } from '@/components/icons/Icons';
...
const RoadmapTab: React.FC<RoadmapTabProps> = ({ blocks, onUpdateBlocks }) => {
    ...
    const handleAddBlock = (type: 'task' | 'milestone' | 'note' | 'decision', position?: { x: number; y: number }) => {
    ...
    const handleMouseMove = useCallback(
        (e: MouseEvent) => {
            ...
            const updatedBlocks = blocks.map(block =>
                block.id === draggingBlock ? { ...block, position: { x: Math.max(0, x), y: Math.max(0, y) } } : block
            );
            onUpdateBlocks(updatedBlocks);
        },
    ...
    const handleDeleteBlock = (id: string) => {
        const updatedBlocks = blocks.filter(block => block.id !== id).map(block => ({
            ...block,
