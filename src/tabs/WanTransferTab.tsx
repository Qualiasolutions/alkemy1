...
import { isWanApiAvailable, transferMotionWan } from '../services/wanService'
...
const WanTransferTab: React.FC<WanTransferTabProps> = ({ scriptAnalysis }) => {
  ...
  const handleTransfer = async () => {
    ...
    try {
      ...
      alert(
        error instanceof Error ? error.message : 'An unknown error occurred during motion transfer.'
      )
    } finally {
      ...
      <div className="flex flex-col items-center justify-center h-[calc(100vh-10rem)] text-center">
        <div className={`p-10 border border-dashed border-[var(--color-border-color)] rounded-2xl`}>
          ...
          <p className="text-lg text-gray-400 max-w-md">
            Please analyze a script in the 'Script' tab to access characters for motion transfer.
          </p>
        </div>
      </div>
    )
    ...
    <div className="h-full flex flex-col">
      <header className="mb-6">
        <h2 className={`text-2xl font-bold mb-1 text-[var(--color-text-primary)]`}>
          Wan 2.5 Motion-Transfer
        </h2>
        <p className={`text-md text-[var(--color-text-secondary)] max-w-3xl`}>
          Upload a reference video of a real actor and transfer their motion and expressions to your
          target Avatar.
        </p>
      </header>
      ...
      <main className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div
          className={`bg-[var(--color-surface-card)] border border-[var(--color-border-color)] rounded-xl p-6`}
        >
