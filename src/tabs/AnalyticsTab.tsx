...
import { generateAnalyticsPDF, generateComparisonPDF } from '@/services/pdfExportService'
...
export default function AnalyticsTab({ scriptAnalysis, projectId }: AnalyticsTabProps) {
  ...
  const [performanceMetrics, setPerformanceMetrics] = useState<TechnicalPerformanceMetrics | null>(
    null
  )
  const [previousQualityReport, setPreviousQualityReport] = useState<CreativeQualityReport | null>(
    null
  )
  ...
  const handleAnalyzeQuality = async () => {
    ...
    try {
      const report = await analyzeCreativeQuality(scriptAnalysis, (progress, message) => {
    ...
    if (format === 'csv' && performanceMetrics) {
      ...
      if (performanceMetrics.renderTimes.imageGeneration.length > 0) {
        ...
        if (performanceMetrics.renderTimes.videoAnimation.count > 0) {
          csvData.push(
            `Video Animation,${performanceMetrics.renderTimes.videoAnimation.avgTime.toFixed(1)},${performanceMetrics.renderTimes.videoAnimation.count}`
          )
        }
      }
      ...
      if (qualityReport) {
        ...
        if (qualityReport.sceneReports.length > 0) {
          ...
          qualityReport.sceneReports.forEach((scene) => {
            csvData.push(
              `${scene.sceneName},${scene.overallScore},${scene.colorConsistency},${scene.lightingCoherence},${scene.lookBibleAdherence}`
            )
          })
        }
        ...
        if (qualityReport.improvementSuggestions.length > 0) {
          ...
          qualityReport.improvementSuggestions.forEach((suggestion) => {
            csvData.push(
              `"${suggestion.issue}","${suggestion.impact}","${suggestion.suggestion}",${suggestion.severity}`
            )
          })
        }
      }
    ...
    <div style={{ padding: '24px', maxWidth: '1400px', margin: '0 auto' }}>
      ...
      {activeTab === 'quality' && (
        <QualityReportView
          ...
          colors={colors}
          showComparison={showComparisonMode}
          previousReport={previousQualityReport}
        />
      )}
  ...
  return (
    <div>
      ...
      >
        ...
        <p style={{ color: colors.text_secondary, fontSize: '18px' }}>
          {qualityIndicator}{' '}
          {qualityLevel.charAt(0).toUpperCase() + qualityLevel.slice(1).replace('-', ' ')}
        </p>
      </motion.div>
      ...
      {report.sceneReports.length > 0 && (
        ...
        >
          ...
          >
            {report.sceneReports.map((scene, _idx) => (
              <div
                key={scene.sceneId}
                style={{
                  background: colors.bg_tertiary,
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    <span style={{ color: colors.text_primary }}>
                      {scene.lightingCoherence}/100
                    </span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ color: colors.text_secondary }}>Look Bible:</span>
                    <span style={{ color: colors.text_primary }}>
                      {scene.lookBibleAdherence}/100
                    </span>
                  </div>
                </div>
              </div>
      ...
      {report.improvementSuggestions.length > 0 && (
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {report.improvementSuggestions.map((suggestion, _idx) => (
              <div
                key={suggestion.id}
                style={{
                  background: colors.bg_tertiary,
...
function PerformanceMetricsView({
  ...
  const totalGenerations =
    metrics.renderTimes.imageGeneration.reduce((sum, m) => sum + m.count, 0) +
  ...
  return (
    <div>
      ...
      >
        ...
        <p style={{ color: colors.text_secondary, fontSize: '18px' }}>
          {totalGenerations} total generations • {metrics.efficiencyMetrics.successRate.toFixed(1)}%
          success rate
        </p>
      </motion.div>
      ...
      {metrics.renderTimes.imageGeneration.length > 0 && (
        ...
        >
          ...
          >
            {metrics.renderTimes.imageGeneration.map((model, _idx) => (
              <div
                key={model.model}
                style={{
                  background: colors.bg_tertiary,
      ...
      {optimizationSuggestions.length > 0 && (
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {optimizationSuggestions.map((suggestion, _idx) => (
              <div
                key={suggestion.id}
                style={{
                  background: colors.bg_tertiary,
                  borderRadius: '8px',
                  padding: '16px',
                  borderLeft: `4px solid ${suggestion.category === 'cost' ? '#DFEC2D' : suggestion.category === 'error' ? '#ef4444' : '#3b82f6'}`,
                }}
              >
                <h4 style={{ color: colors.text_primary, fontSize: '16px', marginBottom: '8px' }}>
                  {suggestion.issue}
