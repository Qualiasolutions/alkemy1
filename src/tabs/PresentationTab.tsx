  ...
  const uniqueImages = useMemo(() => {
    return [
      ...new Set(
        [
          item.imageUrl,
          ...(item.generations || []).map((g) => g.url),
          ...(item.refinedGenerationUrls || []),
    ...
    >
      ...
      >
        ...
        <Button
          onClick={onClose}
          variant="secondary"
          className="absolute top-4 left-4 !text-sm !gap-2 !px-3 !py-2 !bg-black/50 !border-gray-600 hover:!bg-black/80 text-white hover:text-white"
        >
...
const CharacterCastingCard: React.FC<{
  ...
  return (
    <div
      className={`bg-[var(--color-surface-card)] rounded-lg border border-[var(--color-border-color)] p-6 flex flex-col gap-6`}
    >
      <div className="w-full">
        <button
          onClick={onViewImages}
          className="relative aspect-w-3 aspect-h-4 bg-[var(--color-surface-card)] rounded-xl overflow-hidden mb-4 group block w-full text-left border border-[var(--color-border-color)] hover:border-#DFEC2D/50 transition-all"
        >
          {character.imageUrl ? (
            <img
            ...
            <div className="flex items-center justify-center h-full">
              <UsersIcon className={`w-12 h-12 text-[var(--color-text-secondary)]`} />
            </div>
          )}
          <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent flex items-end justify-center pb-6 opacity-0 group-hover:opacity-100 transition-all duration-300">
            <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-#DFEC2D to-#FDE047 text-white font-semibold text-sm shadow-lg shadow-#DFEC2D/30 hover:shadow-#DFEC2D/50 transition-shadow">
...
const LocationCard: React.FC<{ location: AnalyzedLocation }> = ({ location }) => (
  <div
    className={`bg-[var(--color-surface-card)] rounded-xl overflow-hidden border border-[var(--color-border-color)] hover:border-#DFEC2D/50 h-full w-full transition-all duration-300 hover:shadow-lg hover:shadow-#DFEC2D/10`}
  >
...
const StoryboardScene: React.FC<{ scene: AnalyzedScene }> = ({ scene }) => (
  <div>
    <h4 className="text-lg font-bold mb-3 pb-2 border-b border-[var(--color-border-color)] text-[var(--color-text-primary)]">
...
const MoodboardCategory: React.FC<{ title: string; items: MoodboardItem[] }> = ({
  title,
  items,
}) => {
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/5 p-5 shadow-[0_20px_45px_rgba(15,23,42,0.35)]">
      ...
      {board.items.length > 0 ? (
        ...
        <div className="mt-4 flex h-28 items-center justify-center rounded-xl border border-dashed border-white/10 text-xs text-white/40">
          No references yet
        </div>
      )}
    </div>
...
const PresentationTab: React.FC<{ scriptAnalysis: ScriptAnalysis | null }> = ({
  scriptAnalysis,
}) => {
      ...
      <div className="flex flex-col items-center justify-center h-[calc(100vh-10rem)] text-center">
        <div className={`p-10 border border-dashed border-[var(--color-border-color)] rounded-2xl`}>
          ...
          <p className="text-lg text-gray-400 max-w-md">
            Analyze a script to automatically generate a presentation with storyboards, characters,
            and moodboards.
          </p>
        </div>
      </div>
    ...
    <div className="space-y-12">
      ...
      <section>
        ...
        <div className="space-y-8">
          {scriptAnalysis.moodboardTemplates?.some((board) => board.items.length > 0) ? (
            ...
            <div
              className={`flex items-center justify-center h-48 bg-[var(--color-surface-card)] rounded-lg border border-dashed border-[var(--color-border-color)]`}
            >
              <p className={`text-sm text-[var(--color-text-secondary)]`}>
                No references added to moodboard yet.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {locations.map((loc) => (
            <button
              ...
              className="text-left w-full h-full transition-transform duration-200 hover:scale-105"
            >
      ...
      <section>
        ...
        <div className="space-y-8">
          {scenes.map((scene) => (
            <StoryboardScene key={scene.id} scene={scene} />
          ))}
        </div>
      </section>
    </div>
