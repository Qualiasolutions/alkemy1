...
import type { MoodboardItem, MoodboardSection, MoodboardTemplate } from '../types'
...
const _formatAIOutput = (text: string, isDark: boolean): React.ReactNode[] => {
  if (!text) return []
  ...
  return text
    ...
    .map((paragraph, i) => {
      ...
      const isHeading =
        cleaned.startsWith('#') || (cleaned.length < 50 && cleaned === cleaned.toUpperCase())
      ...
      if (isHeading) {
        return (
          <h5
            key={i}
            className={`text-base font-bold mb-2 ${isDark ? 'text-teal-400' : 'text-teal-600'}`}
          >
            {cleaned}
          </h5>
        )
      ...
      if (cleaned.match(/^[-•]\s/)) {
        return (
          <li
            key={i}
            className={`text-sm leading-relaxed ml-4 ${isDark ? 'text-gray-300' : 'text-gray-600'}`}
          >
            {cleaned.replace(/^[-•]\s*/, '')}
          </li>
        )
...
const AIOutputPanel: React.FC<AIOutputPanelProps> = ({ summary, isGenerating, isDark }) => {
  return (
    <div
      className={`rounded-xl overflow-hidden sticky top-4 border backdrop-blur-sm ${
        ...
        {isGenerating ? (
          <div className="space-y-2">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className={`h-2 rounded animate-pulse ${isDark ? 'bg-gray-800' : 'bg-gray-200'}`}
...
const MoodboardTab: React.FC<MoodboardTabProps> = ({
  moodboardTemplates,
  ...
  const [_fullscreenView, _setFullscreenView] = useState(false)
  const [_currentSlideIndex, _setCurrentSlideIndex] = useState(0)
  const [_hoveredImageId, _setHoveredImageId] = useState<string | null>(null)
  ...
  const _cardsContainerRef = useRef<HTMLDivElement>(null)
  ...
  useEffect(() => {
    ...
    if (shouldAutoGenerate) {
      console.log(
        '[MoodboardTab] Auto-generating AI summary at',
        activeMoodboard.items.length,
        'images'
      )
  ...
  const handleTitleChange = useCallback(
    (newTitle: string) => {
      ...
      onUpdateMoodboardTemplates((templates) =>
        templates.map((t) =>
          t.id === activeId ? { ...t, title: newTitle, updatedAt: new Date().toISOString() } : t
        )
      )
    },
  ...
  const handleDescriptionChange = useCallback(
    (newDescription: string) => {
      ...
      onUpdateMoodboardTemplates((templates) =>
        templates.map((t) =>
          t.id === activeId
            ? { ...t, description: newDescription, updatedAt: new Date().toISOString() }
            : t
        )
      )
    },
  ...
  const handleAddTemplate = useCallback(() => {
    console.log(
      '[MoodboardTab] handleAddTemplate called, current templates:',
      moodboardTemplates.length
    )
    ...
    (itemId: string) => {
      ...
      onUpdateMoodboardTemplates((templates) =>
        templates.map((t) => {
          ...
          const updatedCoverId =
            t.coverImageId === itemId ? updatedItems[0]?.id || undefined : t.coverImageId
  ...
  const handleGenerateSummary = useCallback(
    async (isAutoGeneration = false) => {
      ...
      try {
        ...
        console.log(
          `[MoodboardTab] AI summary ${isAutoGeneration ? 'auto-' : ''}generated successfully`
        )
      } catch (error) {
  ...
  const canRegenerate = useMemo(() => {
    ...
    if (activeMoodboard.lastAutoGeneratedAt) {
      const hasNewItems = activeMoodboard.items.some((_item) => {
    ...
    return (
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="text-center max-w-md mx-auto">
          <ImageIcon
            className={`w-20 h-20 mx-auto mb-6 ${isDark ? 'text-gray-600' : 'text-gray-300'}`}
          />
          <h3 className={`text-2xl font-bold mb-3 ${isDark ? 'text-white' : 'text-gray-900'}`}>
            Create Your First Moodboard
          </h3>
          <p className={`text-sm mb-6 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
            Start building visual references for your project. Upload images, search the web, and
            let AI analyze your visual language.
          </p>
                ...
                {isSearching && (
                  <div className="mt-4">
                    ...
                    <div className="mt-2 h-1 bg-gray-800 rounded-full overflow-hidden">
                      <motion.div
                        className={`h-full ${isDark ? 'bg-teal-400' : 'bg-teal-600'}`}
                        initial={{ width: 0 }}
