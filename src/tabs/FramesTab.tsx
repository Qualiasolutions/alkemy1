...
const TimelineButton: React.FC<React.ButtonHTMLAttributes<HTMLButtonElement>> = ({
  children,
  className = '',
  ...props
}) => (
  <button
    className={`px-3 py-1.5 text-xs bg-zinc-800 hover:bg-zinc-700 border border-zinc-700 rounded-lg transition-colors ${className}`}
    {...props}
  >
    {children}
  </button>
...
const Section: React.FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => {
  ...
  return (
    <div className="rounded-xl border border-zinc-800 overflow-hidden">
      <div
        className="flex items-center justify-between px-3 py-2 bg-zinc-900/60 cursor-pointer"
...
const FramesTab: React.FC<{
  ...
  useEffect(() => {
    ...
    if (activeClip) {
      ...
      if (isPlaying && video.paused) {
        video.play().catch((_e) => console.warn('Autoplay was prevented'))
    ...
    const onMouseMove = (e: MouseEvent) => {
      ...
      if (rightDragRef.current) {
        const newRight = Math.min(
          Math.max(180, window.innerWidth - e.clientX),
  ...
  const handleSplitClip = () => {
    ...
    if (timeWithinClip <= 0.1 || timeWithinClip >= activeClip.trimEnd - activeClip.trimStart - 0.1)
      return

    const clip1: TimelineClip = { ...activeClip, trimEnd: splitTimeInSource }
    const clip2: TimelineClip = {
      ...activeClip,
      timelineId: `clip-${activeClip.id}-${Date.now()}`,
      trimStart: splitTimeInSource,
    }
  ...
  const selectedClipData = useMemo(
    () => clips.find((c) => c.timelineId === selectedClipId),
    ...
    <div className="w-full h-full bg-zinc-950 text-zinc-100 select-none overflow-hidden flex flex-col">
      ...
      <div className="flex items-center justify-between px-3 sm:px-4 h-10 border-b border-zinc-800 bg-zinc-900/70 backdrop-blur-sm">
        ...
        <div className="flex items-center gap-2 text-xs">
          <div className="px-2 py-1 rounded bg-zinc-800 border border-zinc-700">
            {timecode(playhead)} / {timecode(totalDuration)}
          </div>
          <TimelineButton
            onClick={onSave}
            className="bg-yellow-700 hover:bg-#FDE047 shadow-lg !py-1 !px-2"
          >
            💾 Save Project
          </TimelineButton>
        </div>
      ...
      <div className="relative w-full flex-1 flex min-h-0">
        ...
        <div
          ...
          className="h-full border-r border-zinc-800 bg-zinc-900/40 flex flex-col"
        >
          <PanelHeader
            title="Project Bin"
            actions={
          ...
          <div className="flex-1 overflow-auto p-2 space-y-1">
            {clips.map((clip) => (
              <div
                key={clip.timelineId}
                className="rounded-md overflow-hidden border border-zinc-800 bg-zinc-900/60 p-2 text-xs"
              >
                <div className="truncate font-semibold" title={clip.description}>
                  {clip.description}
                </div>
        ...
        <div className="flex-1 h-full flex flex-col">
          ...
          <div className="relative flex-1 min-h-[200px] flex flex-col">
            <PanelHeader
              title="Program: Cowboy"
              actions={<TimelineButton className="px-2 py-1">⚙</TimelineButton>}
            />
            <div className="flex-1 grid place-items-center bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-zinc-900 via-zinc-950 to-black">
              <div className="relative w-[72%] max-w-[920px] aspect-video bg-black border border-zinc-800 rounded-xl overflow-hidden shadow-2xl">
                ...
                <div className="absolute bottom-2 left-2 text-xs bg-zinc-900/80 border border-zinc-800 rounded px-2 py-0.5 tabular-nums">
                  {timecode(playhead)}
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-center gap-2 py-3 border-t border-zinc-800 bg-zinc-900/60">
              ...
              <TimelineButton
                ...
                className="bg-yellow-700 hover:bg-#FDE047"
              >
                {isPlaying ? '⏸' : '▶'}
              </TimelineButton>
              <TimelineButton onClick={() => setPlayhead(playhead + 1)}>+1s ⏭</TimelineButton>
              ...
              <div className="flex items-center gap-2 ml-6 text-sm">
                ...
                <input
                  ...
                  step="0.1"
                  value={zoom}
                  onChange={(e) => setUiState((p) => ({ ...p, zoom: parseFloat(e.target.value) }))}
                  className="w-40"
                />
              </div>
            </div>
          ...
          <div
            ...
            className="w-full bg-zinc-900/60 border-t border-zinc-800 flex flex-col"
          >
            <PanelHeader
              title="Timeline: V1 / A1"
              actions={
                <TimelineButton className="!py-1 !px-2" onClick={handleSplitClip}>
                  ✂ Split
                </TimelineButton>
              }
            />
            <div className="flex-1 overflow-auto" ref={timelineContainerRef}>
              <div className="relative h-full" style={{ width: totalDuration * 80 * zoom }}>
                ...
                <div className="relative h-[calc(100%-2rem)]">
                  {/* Playhead line */}
                  <div
                    className="absolute top-0 bottom-0 w-0.5 bg-#DFEC2D shadow-[0_0_8px_rgba(163,230,53,0.7)] z-20"
                    style={{ left: `${playhead * 80 * zoom}px` }}
                  />
                  {/* Video Track */}
                  <div className="h-1/2 border-b border-zinc-800 relative p-1">
                    {clips.map((clip) => {
                      ...
                      return (
                        <div
                          key={clip.timelineId}
                          onClick={() => setSelectedClipId(clip.timelineId)}
                          className={`absolute top-1 bottom-1 rounded-md border text-xs p-1 truncate ${selectedClipId === clip.timelineId ? 'border-amber-400' : 'border-amber-700'} bg-amber-900/50`}
                          style={{ left: `${clipMeta.start * 80 * zoom}px`, width: `${width}px` }}
                        >
                          {clip.description}
                        </div>
                      )
        ...
        <div
          ...
          className="h-full border-l border-zinc-800 bg-zinc-900/40 flex flex-col"
        >
          <PanelHeader
            title="Inspector"
            actions={<TimelineButton className="!py-1 !px-2">⚙</TimelineButton>}
          />
            ...
            {selectedClipData ? (
              <>
                <Section title="Clip">
                  <Field label="Name">
                    <input
                      className="w-full bg-zinc-800 rounded-lg px-2 py-1 border border-zinc-700"
                      value={selectedClipData.description}
                      readOnly
                    />
                  </Field>
                  <Field label="In / Out">
                    <div className="flex gap-2">
                      <input
                        className="w-full bg-zinc-800 rounded-lg px-2 py-1 border border-zinc-700"
                        value={timecode(selectedClipData.trimStart)}
                        readOnly
                      />
                      <input
                        className="w-full bg-zinc-800 rounded-lg px-2 py-1 border border-zinc-700"
                        value={timecode(selectedClipData.trimEnd)}
                        readOnly
                      />
                    </div>
                  </Field>
