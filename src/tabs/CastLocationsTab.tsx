      ...
      >
        ...
        <form onSubmit={(e) => onSubmit(e, name, description)} className="p-6">
          ...
          <div className="mt-6 flex justify-end gap-3">
            <Button type="button" variant="secondary" onClick={onClose} className="!px-6 !py-2.5">
              Cancel
            </Button>
...
const _EditItemModal: React.FC<EditItemModalProps> = ({
  type,
  isOpen,
  currentName,
  currentDescription,
  ...
  return (
    <motion.div
      ...
      className={`group relative rounded-2xl overflow-hidden ${
        ...
        isDark
          ? 'hover:shadow-[var(--color-accent-primary)]/20'
          : 'hover:shadow-[var(--color-accent-primary)]/30'
      }`}
    >
      {/* Status Badges */}
      ...
      <div onClick={onClick} className="cursor-pointer relative">
        <div className="aspect-[4/3] relative overflow-hidden bg-black/5">
          {hasImage ? (
            <>
              ...
              <div
                className={`absolute inset-0 bg-gradient-to-br ${
                  isDark
                    ? 'from-[var(--color-accent-primary)]/10 to-[var(--color-accent-secondary)]/10'
                    : 'from-[var(--color-accent-primary)]/20 to-[var(--color-accent-secondary)]/20'
            ...
            >
              <div className={`${isDark ? 'text-gray-700' : 'text-gray-400'}`}>{icon}</div>
            </div>
          )}
          ...
          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center justify-center">
            <div className="flex flex-col items-center gap-3">
              <motion.div initial={{ scale: 0 }} whileHover={{ scale: 1.2 }} className="text-white">
          ...
          <div className={`pt-3 border-t ${isDark ? 'border-gray-800' : 'border-gray-200'}`}>
              ...
              <div className="flex items-center gap-2">
                ...
                <span
                  className={`text-xs font-medium ${
                    hasImage ? 'text-[#DFEC2D]' : isDark ? 'text-gray-500' : 'text-gray-400'
                  }`}
                >
            ...
            {character && onPrepareIdentity && (
              ...
              >
                ...
                {!identityStatus || identityStatus === 'none'
                  ? 'Train Character'
                  : identityStatus === 'ready'
                    ? 'Retrain Identity'
                    : identityStatus === 'preparing'
...
const CastLocationsTab: React.FC<CastLocationsTabProps> = ({
  characters,
  setCharacters,
  locations,
  setLocations,
  ...
  const [identityModalCharacter, setIdentityModalCharacter] = useState<AnalyzedCharacter | null>(
    null
  )
  const [identityModalLoraImages, setIdentityModalLoraImages] = useState<string[]>([])

  const handleItemUpdateBatch = (
    updater: (prev: AnalyzedCharacter | AnalyzedLocation) => AnalyzedCharacter | AnalyzedLocation
  ) => {
    if (selectedItem?.type === 'character') {
      setCharacters((prevChars) =>
        prevChars.map((c) =>
          c.id === selectedItem.data.id ? (updater(c) as AnalyzedCharacter) : c
        )
      )
    } else if (selectedItem?.type === 'location') {
      setLocations((prevLocs) =>
        prevLocs.map((l) => (l.id === selectedItem.data.id ? (updater(l) as AnalyzedLocation) : l))
      )
    }
  }
    ...
    if (e.target.files?.[0] && itemToUpdate) {
      ...
      reader.onload = (event) => {
        ...
        if (itemToUpdate.type === 'character') {
          setCharacters((chars) =>
            chars.map((c) => (c.id === itemToUpdate.id ? { ...c, imageUrl } : c))
          )
        } else {
          setLocations((locs) =>
            locs.map((l) => (l.id === itemToUpdate.id ? { ...l, imageUrl } : l))
          )
        }
    ...
    setCharacters((prev) =>
      prev.map((c) => {
        if (c.id === characterId) {
          ...
          const imageUrl =
            (identity.referenceImages && identity.referenceImages.length > 0
              ? identity.referenceImages[0]
              : undefined) || c.imageUrl
      ...
      <div className="space-y-12 pb-20">
        ...
        >
          <div className="relative z-10">
            ...
            <p className={`text-base max-w-2xl ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
              Develop the visual identity for your characters and settings. Click any card to enter
              the Generation Studio and create stunning visuals.
            </p>
          </div>
          {/* Decorative Gradient */}
          <div
            className={`absolute -top-8 -right-8 w-64 h-64 rounded-full blur-3xl opacity-20 pointer-events-none ${
              isDark
        ...
        >
          ...
          {characters.length > 0 ? (
            ...
            >
              <UsersIcon
                className={`w-16 h-16 mb-4 ${isDark ? 'text-gray-700' : 'text-gray-400'}`}
              />
              <p
              ...
              <p className={`text-sm mb-6 ${isDark ? 'text-gray-600' : 'text-gray-500'}`}>
